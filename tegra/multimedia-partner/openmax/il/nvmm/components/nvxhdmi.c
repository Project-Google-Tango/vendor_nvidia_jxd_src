...
NvError HdmiNegotiateStreamBuffers(
...
static OMX_ERRORTYPE NvxCameraEnableInputPort(NvxComponent *pNvComp, SNvxCameraData *pNvxCamera)
{
    ...
    if (NvError_Success != HdmiNegotiateStreamBuffers(&pNvxCamera->oSrcCamera, NV_FALSE, &pNvxCamera->s_nvx_port_sc_input, 1))
        return OMX_ErrorUndefined;
...
OMX_ERRORTYPE NvxHdmiInit(OMX_IN  OMX_HANDLETYPE hComponent)
{
    ...
    NVXTRACE((NVXT_CALLGRAPH, NVXT_UNDESIGNATED, "+NvxHdmiInit\n"));

    eError = NvxComponentCreate(hComponent, s_nvx_num_ports, &pNvComp);
    if (eError != OMX_ErrorNone)
    {
        NVXTRACE((NVXT_ERROR, NVXT_CAMERA,"ERROR:NvxHdmiInit:"
            ":eError = %d:[%s(%d)]\n", eError,__FILE__, __LINE__));
        return eError;
    }
    ...
    pNvComp->pPorts[s_nvx_port_still_capture].oPortDef.bEnabled = OMX_FALSE;
    ...
    *data->SensorName = '\0';
    ...
    data->oSrcCamera.BlockSpecific = 1;
    ...
    pNvComp->pComponentName = "OMX.Nvidia.hdmi";
    pNvComp->sComponentRoles[0] = "hdmi.yuv";    // check the correct type
...
static OMX_ERRORTYPE NvxCameraOpen(SNvxCameraData *pNvxCamera, NvxComponent *pComponent)
{
    ...
    // when executing HdmiNegotiateStreamBuffers()
    eError = nvxCameraWaitBufferNegotiationComplete(
        pNvxCamera,
        &portSet,
        NVXCAMERA_NEGOTIATE_BUFFERS_TIMEOUT);
...
NvxCameraGetDzOutputIndex(OMX_S32 nPortIndex)
{
    ...
    if (nPortIndex == (OMX_S32)s_nvx_port_preview)
    {
...
static NvError NvxHdmiUpdateExposureRegions(void *pNvxCam)
{

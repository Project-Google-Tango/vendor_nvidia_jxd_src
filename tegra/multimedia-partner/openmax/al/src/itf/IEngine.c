    ...
    if (NULL == ap->mAndroidBufferQueue.mBufferArray) {
        ...
        switch(ap->mDataSource.mFormat.mMIME.containerType) {
          ...
          case XA_CONTAINERTYPE_MPEG_ES:
            ap->mAndroidBufferQueue.mBufferType = kAndroidBufferTypeEs;
            break;
        ...
        *pRecorder = NULL;
        ...
        if (SL_RESULT_SUCCESS == result) {
            ...
            CAudioRecorder *thiz = (CAudioRecorder *) construct(pCAudioRecorder_class, exposedMask,
                    self, numInterfaces,pInterfaceIds, pInterfaceRequired);
            if (NULL == thiz) {
                result = SL_RESULT_MEMORY_FAILURE;
            } else {
        ...
        *pMetadataExtractor = NULL;
        ...
        if (SL_RESULT_SUCCESS == result) {
            CMetadataExtractor *thiz = (CMetadataExtractor *)
                construct(pCMetadataExtractor_class, exposedMask, self, numInterfaces,
    ...
    const SLInterfaceID *pInterfaceIds, const SLboolean *pInterfaceRequired)
{
    ...
    SL_LOGI("IEngine_CreateExtensionObject ++\n");
    if (NULL == pObject) {
        result = SL_RESULT_PARAMETER_INVALID;
    }
    else if (objectID != NVXA_OBJECTID_DATATAP) {
        *pObject = NULL;
        result = SL_RESULT_FEATURE_UNSUPPORTED;
    }else {
        *pObject = NULL;
        unsigned exposedMask;
        const ClassTable *pCDataTap_class = objectIDtoClass(NVXA_OBJECTID_DATATAP);
        if (pCDataTap_class == NULL)
        {
            SL_LOGE("IEngine_CreateExtensionObject::NUll pointer returned by objectIDtoClass");
            result = SL_RESULT_FEATURE_UNSUPPORTED;
        }
        else
        {
            SL_LOGI("IEngine_CreateExtensionObject::Got Valid Class from objectIDtoClass");
            result = checkInterfaces(pCDataTap_class, numInterfaces, pInterfaceIds,
                pInterfaceRequired, &exposedMask, NULL);
        }
        if (XA_RESULT_SUCCESS == result)
        {
            CNvDataTap *dataTap = (CNvDataTap *)construct(pCDataTap_class, exposedMask, self,
                                                numInterfaces, pInterfaceIds, pInterfaceRequired);
            if (NULL == dataTap)
            {
                SL_LOGE("IEngine_CreateExtensionObject::construct for pCDataTap_class fails");
            ...
            else
            {
                if (pParameters)
                {
                    memcpy( &dataTap->mDataTapCreationParams,pParameters, sizeof(NVXADataTapCreationParameters));
                }
                IObject_Publish(&dataTap->mObject);
                // return the new DataTap object
                *pObject = &dataTap->mObject.mItf;
            }
        }
    }
    SL_LOGI("IEngine_CreateExtensionObject --\n");
        ...
        const XAboolean *pInterfaceRequired)
{
    ...
    if (NULL == pDevice) {
        ...
        *pDevice = NULL;
        unsigned exposedMask;
        const ClassTable *pCCameraDevice_class = objectIDtoClass(XA_OBJECTID_CAMERADEVICE);
        if (NULL == pCCameraDevice_class) {
            result = XA_RESULT_FEATURE_UNSUPPORTED;
        } else {
            result = checkInterfaces(
                pCCameraDevice_class,
                numInterfaces,
        ...
        if (XA_RESULT_SUCCESS == result) {
            CCameraDevice *camera =
                (CCameraDevice *)construct(pCCameraDevice_class, exposedMask,
                     &((CEngine *) ((IXAEngine *) self)->mThis)->mEngine.mItf,
                     numInterfaces, (const SLInterfaceID *)pInterfaceIds, pInterfaceRequired);

            if (NULL == camera) {
                result = XA_RESULT_MEMORY_FAILURE;
            } else {
                camera->mDeviceID = deviceID;
#ifdef ANDROID
                result = android_cameraDevice_create(camera);
#endif
#ifdef LINUX_OMXAL
                result = ALBackendCameraDeviceCreate(camera);
#endif
                if (XA_RESULT_SUCCESS != result) {
                    IObject_Destroy(&camera->mObject.mItf);
                } else {
                    IObject_Publish(&camera->mObject);
                    // return the new Camera Device object
                    *pDevice = (XAObjectItf) &camera->mObject.mItf;
                }
            }
        }
    }
        ...
        *pPlayer = NULL;
        ...
        if (XA_RESULT_SUCCESS == result) {
            ...
            CMediaPlayer *thiz = (CMediaPlayer *) construct(pCMediaPlayer_class, exposedMask,
                    &((CEngine *) ((IXAEngine *) self)->mThis)->mEngine.mItf, numInterfaces,
                    (const SLInterfaceID *) pInterfaceIds, pInterfaceRequired);
            if (NULL == thiz) {
                result = XA_RESULT_MEMORY_FAILURE;
            } else {
                ...
                do {
                    ...
                    result = checkDataSink("pAudioSnk", (const SLDataSink *) pAudioSnk,
                            &thiz->mAudioSink, DATALOCATOR_MASK_OUTPUTMIX
                             | DATALOCATOR_MASK_NULL, DATAFORMAT_MASK_NULL);
                    if (XA_RESULT_SUCCESS != result) {
                        break;
                    }
                    ...
                    result = checkDataSink("pImageVideoSnk", (const SLDataSink *) pImageVideoSnk,
                            ...
                            DATALOCATOR_MASK_NULL | DATALOCATOR_MASK_NATIVEDISPLAY |DATALOCATOR_MASK_EGLSTREAM,
                            DATAFORMAT_MASK_NULL);
                    if (XA_RESULT_SUCCESS != result) {
                        break;
                    }
...
#ifdef LINUX_OMXAL
                    if (pDataSrc)
                        thiz->mDataSrc = *pDataSrc;
                    if (pAudioSnk)
                        thiz->mAudioSnk = *pAudioSnk;
                    if (pImageVideoSnk)
                        thiz->mImageVideoSnk = *pImageVideoSnk;
#endif
                    ...
                    if (XA_DATALOCATOR_ANDROIDBUFFERQUEUE ==
                        ...
                        if (XA_CONTAINERTYPE_MPEG_TS ==
                            ...
                            if (0 != thiz->mStreamInfo.mStreamInfoTable.size())
                            {
                        ...
                        } else if (XA_CONTAINERTYPE_MPEG_ES ==
                                thiz->mDataSource.mFormat.mMIME.containerType) {
                            thiz->mAndroidBufferQueue.mBufferType = kAndroidBufferTypeEs;
                            ...
                            if (0 != thiz->mStreamInfo.mStreamInfoTable.size())
                            {
                                ...
                                XAMediaContainerInformation *containerInfo =
                                        (XAMediaContainerInformation*)
                                            ...
                                            &(thiz->mStreamInfo.mStreamInfoTable.itemAt(0).
                                                    containerInfo);
                                containerInfo->containerType = XA_CONTAINERTYPE_MPEG_ES;
                                // there are no streams at this stage
                                containerInfo->numStreams = 0;
                            }
                        } else {
...
#endif
#ifdef LINUX_OMXAL
                    ALBackendMediaPlayerCreate(thiz);
#endif
                } while (0);
        ...
        const XAboolean *pInterfaceRequired)
{
    ...
    if (NULL == pRecorder) {
        ...
        *pRecorder = NULL;
        unsigned exposedMask;
        const ClassTable *pCMediaRecorder_class = objectIDtoClass(XA_OBJECTID_MEDIARECORDER);
        assert(NULL != pCMediaRecorder_class);
        result = checkInterfaces(
            pCMediaRecorder_class,
            numInterfaces,
            (const SLInterfaceID *)pInterfaceIds,
            pInterfaceRequired,
            &exposedMask,
        ...
        if (XA_RESULT_SUCCESS == result) {
            ...
            // Construct our new MediaRecorder instance
            CMediaRecorder *recorder = (CMediaRecorder *)construct(pCMediaRecorder_class, exposedMask,
                    &((CEngine *) ((IXAEngine *) self)->mThis)->mEngine.mItf, numInterfaces,
                    (const SLInterfaceID *)pInterfaceIds, pInterfaceRequired);
            if (NULL == recorder) {
                result = XA_RESULT_MEMORY_FAILURE;
            } else {
                //Initialize the data source and sink fields
               recorder->mAudioSrc.pFormat = NULL;
               recorder->mAudioSrc.pLocator= NULL;
               recorder->mImageVideoSrc.pFormat =NULL;
               recorder->mImageVideoSrc.pLocator=NULL;
               recorder->mDataSnk.pFormat =NULL;
               recorder->mDataSnk.pLocator=NULL;
#ifdef ANDROID
               recorder->mListener = NULL;
               recorder->mRecorder = NULL;
#endif
                if(pAudioSrc) {
                    recorder->mAudioSrc = *pAudioSrc;
                }
                if(pImageVideoSrc) {
                    recorder->mImageVideoSrc = *pImageVideoSrc;
                }
                if(pDataSnk) {
                    recorder->mDataSnk = *pDataSnk;
                }
                recorder->mode = CMEDIARECORDER_DEFAULT;
                XAboolean snapshotItfRequest = XA_BOOLEAN_FALSE, recordItfRequest = XA_BOOLEAN_FALSE;
                for (XAuint32 i =0; i<numInterfaces; i++) {
                    XAInterfaceID iid = pInterfaceIds[i];
                    if (NULL != iid) {
                        if ((const XAInterfaceID_*)XA_IID_SNAPSHOT == iid) {
                            recorder->mode = CMEDIARECORDER_SNAPSHOT;
                            snapshotItfRequest = XA_BOOLEAN_TRUE;
                         }
                         if (XA_IID_RECORD == iid) {
                            recorder->mode = CMEDIARECORDER_RECORD;
                            recordItfRequest = XA_BOOLEAN_TRUE;
                         }
                    }
                }
#ifdef ANDROID
                if(snapshotItfRequest && recordItfRequest)
                    result = XA_RESULT_PARAMETER_INVALID;
                else {
                    XAuint32 *pDataSinkLocatorType;
                    if(recorder->mode != CMEDIARECORDER_SNAPSHOT)
                    {
                        pDataSinkLocatorType = (XAuint32 *)recorder->mDataSnk.pLocator;
                        switch (*pDataSinkLocatorType)
                        {
                            case XA_DATALOCATOR_ANDROIDBUFFERQUEUE:
                            {
                                XADataLocator_AndroidBufferQueue *bqLocator;
                                bqLocator = (XADataLocator_AndroidBufferQueue *)
                                                recorder->mDataSnk.pLocator;
                                recorder->mAndroidBufferQueue.mNumBuffers =
                                    bqLocator->numBuffers;

                                if (recorder->mAndroidBufferQueue.mNumBuffers >= 256) {
                                    result = XA_RESULT_MEMORY_FAILURE;
                                    break;
                                }

                                // initialize ABQ buffer type
                                XADataFormat_MIME *mime = (XADataFormat_MIME*)(recorder->mDataSnk.pFormat);
                                assert(XA_DATAFORMAT_MIME == mime->formatType);

                                if (XA_CONTAINERTYPE_MPEG_TS == mime->containerType) {
                                    recorder->mAndroidBufferQueue.mBufferType = kAndroidBufferTypeMpeg2Ts;

                                } else if (XA_CONTAINERTYPE_MPEG_ES == mime->containerType) {

                                    recorder->mAndroidBufferQueue.mBufferType = kAndroidBufferTypeEs;
                                } else {
                                    recorder->mAndroidBufferQueue.mBufferType = kAndroidBufferTypeInvalid;
                                    SL_LOGE("Invalid buffer type in Android Buffer Queue");
                                    result = SL_RESULT_CONTENT_UNSUPPORTED;
                                }

                                XAuint16 mNumBuffers =
                                    recorder->mAndroidBufferQueue.mNumBuffers;
                                recorder->mAndroidBufferQueue.mBufferArray =
                                    (AdvancedBufferHeader *) malloc((mNumBuffers + 1) *
                                                            sizeof(AdvancedBufferHeader));

                                if (NULL == recorder->mAndroidBufferQueue.mBufferArray) {
                                    result = XA_RESULT_MEMORY_FAILURE;
                                    break;
                                }
                                else {
                                    for (XAuint16 i=0 ; i<(mNumBuffers + 1) ; i++) {
                                        recorder->mAndroidBufferQueue.mBufferArray[i].mDataBuffer = NULL;
                                        recorder->mAndroidBufferQueue.mBufferArray[i].mDataSize = 0;
                                        recorder->mAndroidBufferQueue.mBufferArray[i].mDataSizeConsumed = 0;
                                        recorder->mAndroidBufferQueue.mBufferArray[i].mBufferContext = NULL;
                                        recorder->mAndroidBufferQueue.mBufferArray[i].mBufferState =
                                                XA_ANDROIDBUFFERQUEUEEVENT_NONE;
                                        switch (recorder->mAndroidBufferQueue.mBufferType) {
                                          case kAndroidBufferTypeMpeg2Ts:
                                          case kAndroidBufferTypeEs:
                                            recorder->mAndroidBufferQueue.mBufferArray[i].mItems.mTsCmdData.
                                                    mTsCmdCode = ANDROID_MP2TSEVENT_NONE;
                                            recorder->mAndroidBufferQueue.mBufferArray[i].mItems.mTsCmdData.
                                                    mPts = 0;
                                            break;
                                ...
                                recorder->mAndroidBufferQueue.mFront =
                                    recorder->mAndroidBufferQueue.mBufferArray;
                                recorder->mAndroidBufferQueue.mRear =
                                    recorder->mAndroidBufferQueue.mBufferArray;
                                break;
                            }
                        }
                        if (result == XA_RESULT_SUCCESS) {
                            result = android_mediaRecorder_create(recorder);
                        }
                    }
                    else
                        result = android_cameraSnapshot_create(recorder);

                    if (XA_RESULT_SUCCESS != result) {
                        IObject_Destroy(&recorder->mObject.mItf);
                    } else {
                        IObject_Publish(&recorder->mObject);
                        // return the new media recorder object
                        *pRecorder = (XAObjectItf) &recorder->mObject.mItf;
                    }
                }
#endif
#ifdef LINUX_OMXAL
                result = ALBackendMediaRecorderCreate(recorder);
                if (XA_RESULT_SUCCESS != result) {
                    IObject_Destroy(&recorder->mObject.mItf);
                } else {
                    IObject_Publish(&recorder->mObject);

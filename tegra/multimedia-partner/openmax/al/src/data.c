        ...
        SLuint32 allowedDataLocatorMask)
{
    ...
    if (NULL == pLocator) {
        ...
        case XA_DATALOCATOR_NATIVEDISPLAY:
            ...
            // hDisplay can be non-NULL for Video Post processing use case
        ...
        case XA_DATALOCATOR_EGLSTREAM:
            pDataLocator->mEGLStream = *(XADataLocator_EGLStream *)pLocator;
            if (pDataLocator->mEGLStream.pEGLStream== NULL) {
                SL_LOGE("%s: pEGLStream must be non-NULL ", name);
                result = SL_RESULT_PARAMETER_INVALID;
            }
            if (pDataLocator->mEGLStream.pEGLDisplay == NULL) {
                SL_LOGE("%s: pEGLDisplay must be non NULL", name);
                result = SL_RESULT_PARAMETER_INVALID;
            }
            break;
...
static void freeDataLocator(DataLocator *pDataLocator)
{
    ...
    case SL_DATALOCATOR_ANDROIDFD:
        break;
    case XA_DATALOCATOR_EGLSTREAM:
        SL_LOGV("freeDataLocator: type: XA_DATALOCATOR_EGLSTREAM: nothing to be done here\n");
        break;
#endif
    default:
        ...
        SLuint32 allowedDataFormatMask)
{
    ...
    if (SL_RESULT_SUCCESS == result) {
        ...
        if (!(allowedDataFormatMask & actualMask)) {
            SL_LOGE("%s: data format %d not allowed 0x%x \n", name, formatType,
...
#endif
    case XA_DATALOCATOR_EGLSTREAM:
        allowedDataFormatMask = 1L << XA_DATAFORMAT_RAWIMAGE;
        allowedDataFormatMask |= DATAFORMAT_MASK_NULL;
        break;
    default:
        // invalid data locator type is caught earlier

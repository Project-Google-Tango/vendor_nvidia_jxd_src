...
#if (BUILD_FOR_AOS == 0)
#define LOG_TAG "CAM_focuser_dw9718"
...
static NvBool setPosition(FocuserCtxt *ctxt, NvS32 Position)
{
   if (Position < ctxt->Config.pos_actual_low && Position > ctxt->Config.pos_actual_high)
    {
        FOCUSER_DB_PRINTF("DW9718 %s: position %d out of bounds\n", __func__, Position);
        return NV_FALSE;
    }
    FOCUSER_DB_PRINTF("DW9718 %s: position = %d\n", __func__, Position);

    if (ioctl(ctxt->focuser_fd, DW9718_IOCTL_SET_POSITION, (void *)(Position)) < 0)
    {
        NvOsDebugPrintf("DW9718 %s: ioctl to set focus failed - %s\n",
                __FILE__,
                strerror(errno));
        return NV_FALSE;
    }
...
static void FocuserDW9718_Close(NvOdmImagerHandle hImager);

/**
 * Focuser_Open
 * Initialize Focuser's private context.
 */
static NvBool FocuserDW9718_Open(NvOdmImagerHandle hImager)
{
    FocuserCtxt *ctxt = NULL;

    NvOsDebugPrintf("DW9718_Focuser_Open\n");
...
#ifdef O_CLOEXEC
    ctxt->focuser_fd = open("/dev/dw9718", O_RDWR|O_CLOEXEC);
#else
    ctxt->focuser_fd = open("/dev/dw9718", O_RDWR);
#endif
    ...
    if (ioctl(ctxt->focuser_fd, DW9718_IOCTL_GET_CONFIG, &ctxt->Config) < 0)
    {
 ...
 */
static NvBool FocuserDW9718_SetPowerLevel(NvOdmImagerHandle hImager,
                NvOdmImagerPowerLevel PowerLevel)
{
    ...
    FOCUSER_DB_PRINTF("DW9718 Focuser_SetPowerLevel (%d)\n", PowerLevel);
 ...
 */
static void FocuserDW9718_GetCapabilities(NvOdmImagerHandle hImager,
 ...
 */
static NvBool FocuserDW9718_SetParameter(NvOdmImagerHandle hImager,
            ...
            NvS32 SizeOfValue, const void *pValue)
{
    ...
    FOCUSER_DB_PRINTF("DW9718 Focuser_SetParameter (%d)\n", Param);
    ...
    switch (Param)
    {
        ...
        case NvOdmImagerParameter_FocuserCapabilities:
        {
            ...
            FOCUSER_DB_PRINTF("DW9718 %s: pCaps->afConfigSetSize %d",
                        __FUNCTION__, pCaps->afConfigSetSize);
            FOCUSER_DB_PRINTF("DW9718 %s: posActualLow %d high %d positionWorkingLow %d"
            ...
            for (i = 0; i < pCaps->afConfigSetSize; i++)
            {
                ...
                FOCUSER_DB_PRINTF("DW9718 i %d posture %d macro %d hyper %d inf %d "
                ...
                for (j = 0; j < pCaps->afConfigSet[i].num_dist_pairs; j++)
                {
                    ...
                    FOCUSER_DB_PRINTF("DW9718 i %d j %d fdn %d distance %d", i, j,
                                pCaps->afConfigSet[i].dist_pair[j].fdn,
                                pCaps->afConfigSet[i].dist_pair[j].distance);
                }
            }

            if (ioctl(pContext->focuser_fd, DW9718_IOCTL_SET_CONFIG,
                    &pContext->Config) < 0)
            {
 ...
 */
static NvBool FocuserDW9718_GetParameter(NvOdmImagerHandle hImager,
                NvOdmImagerParameter Param,
                NvS32 SizeOfValue,
                void *pValue)
{
    ...
    FOCUSER_DB_PRINTF("DW9718 Focuser_GetParameter (%d)\n", Param);
    ...
    switch(Param) {
        ...
        case NvOdmImagerParameter_FocuserCapabilities:
        {
            ...
            FOCUSER_DB_PRINTF("DW9718 %s: Actual low %d high %d, Working low %d "
...
/**
 * FocuserDW9718_GetHal.
 * return the hal functions associated with focuser
 */
NvBool FocuserDW9718_GetHal(NvOdmImagerHandle hImager)
{
    if (!hImager || !hImager->pFocuser)
        return NV_FALSE;

    hImager->pFocuser->pfnOpen = FocuserDW9718_Open;
    hImager->pFocuser->pfnClose = FocuserDW9718_Close;
    hImager->pFocuser->pfnGetCapabilities = FocuserDW9718_GetCapabilities;
    hImager->pFocuser->pfnSetPowerLevel = FocuserDW9718_SetPowerLevel;
    hImager->pFocuser->pfnSetParameter = FocuserDW9718_SetParameter;
    hImager->pFocuser->pfnGetParameter = FocuserDW9718_GetParameter;

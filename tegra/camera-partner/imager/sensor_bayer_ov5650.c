...
static SensorSetModeSequence g_SensorBayerSetModeSequenceList[] =
{
    ...
    {{{2592, 1944}, {HACT_START_FULL,  VACT_START_FULL}, 14, 1.0, 20.0, NvOdmImagerNoCropMode,
                {{600, 12, 600 + 2592, 12 + 1952}, {1.f, 1.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_FULL},
    ...
    {{{1296, 972},  {HACT_START_QUART, VACT_START_QUART}, 30, 1.0, 20.0, NvOdmImagerNoCropMode,
                {{0, 0, 0, 0}, {2.f, 2.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_QUART},
    ...
    {{{1920, 1080},  {0, 0}, 30, 1.0, 20.0, NvOdmImagerPartialCropMode,
                {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_1080P_NEW},
    ...
    {{{2080, 1164},  {HACT_START_QUART, VACT_START_QUART}, 30, 1.0, 20.0, NvOdmImagerPartialCropMode,
                {{0, 0, 0, 0}, {1.f, 1.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_1080P},
    ...
    {{{320, 240},  {HACT_START_QVGA, VACT_START_QVGA}, 120, 1.0, 20.0, NvOdmImagerPartialCropMode,
                {{0, 0, 0, 0}, {2.f, 2.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_QVGA},
...
static SensorSetModeSequence g_SensorBayerSetModeSequenceStereoList[] =
{
     ...
     // pll_multiplier (pll multiplier should come from sensor vendor.  MCLK*pll_multiplier = sensor_output_clk (MIPI_CLK or PCLK)
    {{{2592, 1944}, {HACT_START_FULL,  VACT_START_FULL},  14, 1.0, 20.0, NvOdmImagerNoCropMode,
                {{0, 0, 0, 0}, {1.f, 1.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_FULL},
                NULL, &ModeDependentSettings_OV5650_2592X1944}, // Full Mode, sensor is outputing 2592X1944.
#if 0 //Bug# 941425
    {{{1296, 972},  {HACT_START_QUART, VACT_START_QUART}, 30, 1.0, 20.0, NvOdmImagerNoCropMode,
                {{0, 0, 0, 0}, {0.f, 0.f}}, NvOdmImagerModeType_Normal, SENSOR_BAYER_DEFAULT_LINE_LENGTH_QUART},
                NULL, &ModeDependentSettings_OV5650_1296X972}, // preview/encoding mode, same aspect ratio as above one.
#endif
#if SUPPORT_1080P_RES
    {{{2080, 1164},  {HACT_START_QUART, VACT_START_QUART}, 30, 1.0, 20.0, NvOdmImagerPartialCropMode,
...
#ifdef O_CLOEXEC
            pContext->camera_fd = open("/dev/ov5650", O_RDWR|O_CLOEXEC);
#else
...
#if (BUILD_FOR_AOS == 0)
                    ret = ioctl(pContext->camera_fd, OV5650_IOCTL_TEST_PATTERN, TEST_PATTERN_COLORBARS);
                    if (ret < 0)
                    {
                        NvOsDebugPrintf("ioctl test pattern failed  %s\n", strerror(errno));
                        return NV_FALSE;
                    }
#endif
                }
    ...
    switch(Param)
    {
        ...
        case NvOdmImagerParameter_FuseID:
            ...
            // code to read the ID from sensor
            {
                ...
                ret = ioctl(pContext->camera_fd, OV5650_IOCTL_GET_FUSEID, &fuse_id);
                if (ret < 0)
                {

...
static int NvCameraHal_getDisplayOrientation();
        ...
        NvBool bUSB)
{
    ...
    *sCameraInfo = NULL;
    ...
    if (!hConfFile) {
        ALOGD("No file /etc/nvcamera.conf");
    ...
    {
        ALOGE("Can't read /etc/nvcamera.conf");
        goto cleanup;
    }
    while (!feof(hConfFile))
    {
        ...
        if ((buf[0]!='#')&&(strncmp(buf,"//",2))&&(buf[0]!='\0'))
        {
            ...
            if (!foundVersion)
            {
                ...
                if (err != NvSuccess)
                {
                    ALOGE("in /etc/nvcamera.conf: missing version (%d)", line);
                    goto cleanup;
                }
                else
            ...
            else
            {
                ...
                if (err == NvSuccess)
                {
                    ...
                    {
                        ALOGE("in /etc/nvcamera.conf(%d): invalid entry, cameraType %d", line, cameraType);
                    }
                }
                else
                {
                    ALOGWW("in /etc/nvcamera.conf(%d): malformed line.", line);
                    goto cleanup;
                }
            }
        }
...
checkusb:
    if(bUSB)
    {
        ...
        *sCameraInfo = ret;
        (*sCameraInfo)[numCameras-1].deviceLocation = (char *)NvOsAlloc((strlen("Default")+1)*sizeof(char));
        if ((*sCameraInfo)[numCameras-1].deviceLocation == NULL)
            goto cleanup;
        strncpy((*sCameraInfo)[numCameras-1].deviceLocation, "Default", strlen("Default")+1);
#ifdef FRAMEWORK_HAS_EXTENDED_CAM_INFO_SUPPORT
        (*sCameraInfo)[numCameras-1].camInfo.facing = CAMERA_FACING_UNKNOWN;
#endif
        (*sCameraInfo)[numCameras-1].camInfo.orientation = 0;
        (*sCameraInfo)[numCameras-1].camType = NVCAMERAHAL_CAMERATYPE_USB;
    }
    ...
    NvCameraHal_CameraType *cameraType)
{
    ...
    *cameraID = 0;
    ...
    ret = NvCameraHal_parseNvS16(&pbuf, &orient);
    if (ret != NvSuccess)
        return NvError_BadParameter;
    ...
    // is 270 or 180.
    *orientation = NvCameraHal_getDisplayOrientation();

    if((*orientation == 0) && (orient == 180))
    {
        *orientation = 180;
    }
    else if((*orientation == 90) && (orient == 270))
    {
        *orientation = 270;
    }
...
static int NvCameraHal_getDisplayOrientation()
{
    ...
    if (!file)
    {
        ALOGE("%s: Failed to read tegra display device.", __FUNCTION__);

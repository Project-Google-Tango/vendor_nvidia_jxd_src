...
#define NVCAMERAISP_EXPOSURE_AUTO   NV_SFX_MINUS_ONE
...
#define NVCAMERAISP_ISO_3200   (3200L)
    ...
    /**
     * With one call, the AE override sets the gains, exposure,
     * short exposure, and/or exposure ratio.
     */
...
// Attributes to Set/Get
    ...
    /**
     * Read-only. Returns the default YUV conversion matrix used by nvmm_camera.
...
#define NVCAMERAISP_FLASHSTATUS_REDEYE                  (0x40)
     ...
     * When the HAL configures bracket capture, nvmm_camera sets this flag.
     * Setting the flag ensures that AE bracketed burst mode is enabled.
     * This flag differs from ::NvCameraIspAeHwSettings.BracketedBurstMode, which
     * indicates whether the application has enable bracketed burst mode.
     *
     * - Parameter type: NvBool.
     */
...
/*
 * Defines the output of AE in terms of
 * sensor and ISP settings. The values in this struct are
 * for internal use only.
 * (Because this structure is for internal use only,
 * it is omitted from the API Reference generated for customers.)
 */
typedef struct
{

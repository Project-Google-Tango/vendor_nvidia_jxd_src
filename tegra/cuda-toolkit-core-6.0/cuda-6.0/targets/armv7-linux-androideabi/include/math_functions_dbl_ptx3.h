...
static __forceinline__ void sincospi(double a, double *sptr, double *cptr)
{
  ...
  if (2*((unsigned)__double2hiint(a)) > 0x86800000) {
...
static __forceinline__ double acos(double a)
{
  ...
  if (ihi < 0x3fe26666) {  
    ...
    double p, r, y;
    y = 1.0 - t0;
    r = __internal_fast_rsqrt (__internal_half(y));
...
static __forceinline__ double acosh(double a)
{
  double t;
  t = a - 1.0;
  if (fabs(t) > CUDART_TWO_TO_52 || a < 0.0) {
...
static __forceinline__ double hypot(double a, double b)
{
  ...
  /* extract exponents */
  expa = ((__double2hiint (a) >> 20) & 0x7ff) - 1023;
  expb = ((__double2hiint (b) >> 20) & 0x7ff) - 1023;
  
  /* scale down */
  x = __internal_exp_scale (a, -expb);
  y = __internal_exp_scale (b, -expb);
  ...
  /* scale up */
  t = __internal_exp_scale (t, expb);

  /* if both a & b were zero, now b := 0 */
  fb = fa + fb;

  /* fixup for NaN, both zero, large differences */
  if (!(fb > a) || expa - expb > 53) {
    t = fb;
  }

  t = (a == CUDART_INF) ? a : t; /* fixup for infinity */
  return t;
}
...
static __forceinline__ double rhypot(double a, double b)
{
  ...
  t = (b > CUDART_ZERO) ? t : b; /* fixup for NaNs */
  t = (a == CUDART_INF) ? 0.0 : t; /* fixup for inf */
  t = (b == CUDART_ZERO) ? CUDART_INF : t; /* fixup for both zero */
  return t;
}
...
static __forceinline__ double rcbrt(double a)
{
  ...
  if ((a == 0.0) || !(__isfinited(a))) {
    t = (a == 0.0) ? CUDART_INF : CUDART_ZERO;
    if (__isnand(a)) {
      t = a + a;
    }
...
static __forceinline__ double erf(double a)
{
  ...
  if (t >= 1.0) {
    ...
    r = 1.0 - __internal_fast_exp (r);
    if ((unsigned int)__double2hiint(t) >= 0x4017b000u) { /* 5.921875 */
      r = 1.0;
    }    
...
static __forceinline__ double erfcinv(double a)
{
  ...
  if ((a >= 9.6569334993691083e-4) && (a <= 1.9990343066500631e+0)) {
    ...
    if (a > 1.0) p = t;
    if (p >= 1e-100) {
      /* approximation on [1e-100, 9.6569334993691083e-4] */
      ...
      /* approximation on [1e-325, 1e-100] */
      t = log (p);
      t = rsqrt (-t);
...
static __forceinline__ double frexp(double a, int *b)
{
  ...
  expo = ((unsigned int)__double2hiint(a) >> 20) & 0x7ff;

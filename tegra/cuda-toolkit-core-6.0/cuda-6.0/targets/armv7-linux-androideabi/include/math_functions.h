 ...
 *
 * \return
 * Returns a nonzero value if and only if \p a is negative.  Reports
 ...
 *
 * \return
 * Returns a nonzero value if and only if \p a is a infinite value.
 */
 ...
 *
 * \return
 * Returns a nonzero value if and only if \p a is a NaN value.
 */
 ...
 *
 * \return
 * Returns a nonzero value if and only if \p a is a finite value.
 */
...
static __forceinline__ float acoshf(float a)
{
  ...
  t = a - 1.0f;
  if (t > CUDART_TWO_TO_23_F || (t < 0.0f)) {
...
static __forceinline__ float atanhf(float a)
{
  ...
  t = 2.0f * fa;
  s = 1.0f - fa;
  asm ("div.full.f32 %0,%1,%2;" : "=f"(t) : "f"(t), "f"(s));
  t = 0.5f * log1pf(t);
...
static __forceinline__ float rhypotf(float a, float b) 
{
  ...
  t = (v == 0.0f) ? CUDART_INF_F : t; /* fixup for both zero */
  if ((!(fa <= CUDART_INF_F)) || (!(fb <= CUDART_INF_F))) {
    t = a + b; /* fixup for NaNs */
  }
  t = (v == CUDART_INF_F) ? 0.0f : t; /* fixup for infinities */
  return t;
}
...
static __forceinline__ float normcdfinvf(float a)
{
  return -CUDART_SQRT_TWO_F * erfcinvf (a + a);
}

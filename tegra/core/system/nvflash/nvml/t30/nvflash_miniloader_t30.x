...
{
/* . = 0x108000; */
/* To avoid the fill areas, the aligned *
*  buffers are packed together          */
. = 0x4003d000;
  USB_BUFFER1 :
  {
    *(.bss.s_UsbBuffer)
  ...
  USB_BUFFER2 :
  {
    *(.bss.s_UsbDescriptorBuf)
  }
  .stack :
  {
    Image$$ProgramStackRegion$$Base = .;
    _stack = ABSOLUTE(.);
    *(.stack)
    . += 0x400;
    Image$$ProgramStackRegionEnd$$Base = .;
  }
. = 0x4000A000;
  Image$$MainRegion$$Base = .;
  ...
  miniloader_end = .;
  ASSERT(miniloader_end <= 0x4003d000, "Miniloader overwraps with Buffers and keyschedule WAR work area")

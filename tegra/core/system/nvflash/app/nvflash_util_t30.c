...
void NvFlashUtilT30DumpBit(NvU8 * BitBuffer, const char *dumpOption);
static NvU32 NvFlashUtilT30GetBctSize(void);
static NvBool NvFlashUtilT30MungeBct(const char * bct_file, const char * tmp_file);
NvU32 NvFlashUtilT30GetBlEntry(void);
NvU32 NvFlashUtilT30GetBlAddress(void);
void NvFlashUtilT30SetNCTCustInfoFieldsInBct(NctCustInfo NCTCustInfo,
                                   NvU8 * BctBuffer);
...
DumpStructureOffsets(void)
{
    ...
    PRINT_OFFSET(NvBootConfigTable, CryptoHash, "           ");
...
DumpBasicContents(void)
{
    ...
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, FuseDataWidth);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, FuseNumAddressCycles);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, FuseReadBctFromSda);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, FusePinmuxSelection);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, FusePinOrder);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, DiscoveredDataWidth);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, IdRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, NumPagesRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MLba, NumUncorrectableErrorPages);

    NvAuPrintf("\n");

    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, FuseBlockSize);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, FusePageSize);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, ClockSource);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, ClockDivider);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, NumPagesRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, LastBlockRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, LastPageRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, BootStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, InitStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, MuxOneNand, ParamsValidated);
    ...
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FuseCardType);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FuseVoltageRange);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FuseDisableBootMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FusePinmuxSelection);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FuseDdrMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sdmmc, FuseSdCntrlSelection);
    ...
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, IsFuse32BitMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, IsFuseNonMuxedMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, ClockSource);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, ClockDivider);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, BlockSize);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, PageSize);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, NumPagesRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, LastBlockRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, LastPageRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, InitStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Snor, ParamsValidated);
    ...
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataClockSource);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataClockDivider);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataOobClockSource);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataOobClockDivider);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, SataTransferMode);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, ParamsValidated);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, InitStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, NumPagesRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, LastBlockRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, LastPageRead);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, PortStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciSataBuffersBase);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciDataBuffersBase);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciDmaStatus);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciDataXmissionError);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciCommandError);
    PRINT_SECDEVSTATUS_ITEM(NVU32, Sata, AhciTfdError);
...
AnalyzeBctData(void)
{
    ...
    else
    {
        ...
        NvAuPrintf("    -> Cold boot was not attempted or there was a device error during initialization.\n");
        return;
    }
...
AnalyzeBlData(void)
{
    ...
    for (BlIndex = 0; BlIndex < NVBOOT_MAX_BOOTLOADERS; BlIndex++)
    {
        ...
        if (Bct && (BlIndex > 0) && Bct->BootLoader[BlIndex].Version != CurrentVersion)
        {
        ...
        switch (Bit->BlState[BlIndex].Status)
        {
            ...
            case NvBootRdrStatus_ValidationFailure:
                NvAuPrintf("        This BL was unable to load due to validation faliure.\n");
                break;
            case NvBootRdrStatus_DeviceReadError:
                NvAuPrintf("        This BL was unable to load due to unrecoverable device read error(s).\n");
                break;
        ...
        {
            NvAuPrintf("        This BL had at least one device read error at block %d, page %d.\n",
        ...
        {
            NvAuPrintf("        This BL had at least one correctable ECC error at block %d, page %d.\n",
...
void
NvFlashUtilT30DumpBit(NvU8 * BitBuffer, const char *dumpOption)
{
...
NvBool
NvFlashUtilT30MungeBct( const char *bct_file, const char *tmp_file )
{
    NvError e;
...
NvU32
NvFlashUtilT30GetBctSize(void)
{
    return sizeof(NvBootConfigTable);
}

NvU32
NvFlashUtilT30GetBlEntry(void)
{
    return NVFLASH_BL_ENTRY;
}

NvU32
NvFlashUtilT30GetBlAddress(void)
{
    return NVFLASH_BL_ADDRESS;
}

void
NvFlashUtilT30SetNCTCustInfoFieldsInBct(NctCustInfo NCTCustInfo,
    NvU8 * BctBuffer)
{
    /* NCT implementation being utilized for Loki only as of now */
}

NvBool NvFlashUtilGetT30Hal(NvFlashUtilHal *pHal)
{
    if (pHal->devId == 0x30)
    {
        pHal->MungeBct = NvFlashUtilT30MungeBct;
        pHal->GetBctSize = NvFlashUtilT30GetBctSize;
        pHal->GetBlEntry= NvFlashUtilT30GetBlEntry;
        pHal->GetBlAddress = NvFlashUtilT30GetBlAddress;
        pHal->GetDumpBit = NvFlashUtilT30DumpBit;
        pHal->SetNCTCustInfoFieldsInBct = NvFlashUtilT30SetNCTCustInfoFieldsInBct;
        return NV_TRUE;
    }
    return NV_FALSE;
}

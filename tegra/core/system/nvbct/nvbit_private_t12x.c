...
static NvBool NvBitPrivIsValidBitT12x(NvBootInfoTable *pBit)
{
    if (((pBit->BootRomVersion == NVBOOT_VERSION(0x40 ,0x01))
        || (pBit->BootRomVersion == NVBOOT_VERSION(0x40 ,0x02)))
        &&   (pBit->DataVersion    == NVBOOT_VERSION(0x40 ,0x01))
        &&   (pBit->RcmVersion     == NVBOOT_VERSION(0x40 ,0x01))

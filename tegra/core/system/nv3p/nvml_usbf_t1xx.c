...
#include "include/nvboot_util_int.h"
...
#define NV3P_TRANSFER_EP0_BUFFER_OFFSET NV3P_TRANSFER_EP0_BUFFER_OFFSET_T1XX
#define NV3P_TRANSFER_EP_BULK_BUFFER_OFFSET NV3P_TRANSFER_EP_BULK_BUFFER_OFFSET_T1XX
#define NV3P_USB_BUFFER_OFFSET NV3P_USB_BUFFER_OFFSET_T1XX
...
// LFCON0  10                 10                10               10                 10              10              10
///////////////////////////////////////////////////////////////////////////////
static const UsbPllClockParams s_UsbPllBaseInfo[NvBootClocksOscFreq_MaxVal] =
{
    //DivN, DivM, DivP, CPCON,  LFCON
    {0x3C0, 0x0D, 0x00, 0xC, 2}, // For NvBootClocksOscFreq_13,
    {0x190, 0x07, 0x00, 0x5, 2}, // For NvBootClocksOscFreq_16_8
    {    0,    0,    0,   0, 0}, // dummy field
    {    0,    0,    0,   0, 0}, // dummy field
    {0x0C8, 0x04, 0x00, 0x3, 2}, // For NvBootClocksOscFreq_19_2
    {0x0C8, 0x04, 0x00, 0x3, 2}, // For NvBootClocksOscFreq_38_4,
    {    0,    0,    0,   0, 0}, // dummy field
    {    0,    0,    0,   0, 0}, // dummy field
    {0x3C0, 0x0C, 0x00, 0xC, 2}, // For NvBootClocksOscFreq_12
    {0x3C0, 0x0C, 0x00, 0xC, 2}, // For NvBootClocksOscFreq_48,
    {    0,    0,    0,   0, 0}, // dummy field
    {    0,    0,    0,   0, 0}, // dummy field
    {0x3C0, 0x1A, 0x00, 0xC, 2}  // For NvBootClocksOscFreq_26
};
...
static void NvMlPrivUsbInitT1xx(void)
{
    NvU32 RegVal = 0;
    ...
    OscFreq = NvBootClocksGetOscFreqT1xx();

    Misc1 = NV_DRF_NUM(MISC1, CLK_RST_CONTROLLER_PLLU_MISC, PLLU_CPCON,
        s_UsbPllBaseInfo[OscFreq].CPCON);
    Misc2 = NV_DRF_NUM(MISC2, CLK_RST_CONTROLLER_PLLU_MISC, PLLU_LFCON,
        s_UsbPllBaseInfo[OscFreq].LFCON);

    // Enable PLL U for USB
    NvBootClocksStartPllT1xx(NvBootClocksPllId_PllU,
    ...
    NvOsWaitUS(PlluStableTime);

    RegVal = NV_READ32(NV_ADDRESS_MAP_CAR_BASE +
            CLK_RST_CONTROLLER_PLLU_BASE_0);
    RegVal |= NV_DRF_NUM(CLK_RST_CONTROLLER, PLLU_BASE, PLLU_OVERRIDE, 1);
    NV_WRITE32(NV_ADDRESS_MAP_CAR_BASE + NVBOOT_CLOCKS_PLL_BASE(
       NvBootClocksPllId_PllU), RegVal);

    NvBootClocksSetEnableT1xx(NvBootClocksClockId_UsbId, NV_TRUE);

    // deassert reset to XUSB PAD Control block.
#if !defined (ENABLE_T148) && !defined (ENABLE_T124)
    NvBootResetSetEnableT1xx(NvBootResetDeviceId_XUsbId, NV_FALSE);
#elif defined (ENABLE_T124)
    NvBootResetSetEnableT124(NvBootResetDeviceId_XUsbId, NV_FALSE);
#endif
   ...
   // UTMIPLL_IDDQ_OVERRIDE_VALUE=0 and UTMIPLL_IDDQ_SWCTL=1 should be
    ...
    /* To Use the B Session Valid for cable detection logic,
     * Set VBUS_SENSE_CTL bit in USB_LEGACY_CTRL register
     */
    USBIF_REG_UPDATE_DEF(UsbBase,
                         USB1_LEGACY_CTRL,
                         USB1_VBUS_SENSE_CTL,
...
static NvBool NvMlPrivUsbfIsCableConnectedT1xx(NvBootUsbfContext *pUsbContext)
{
    ...
    **/
    if(USBIF_DRF_VAL(USB_PHY_VBUS_SENSORS,
                    B_SESS_VLD_SW_EN,
...
static NvBootError NvMlPrivUsbfHwEnableControllerT1xx(void)
{
...
static NvBootError NvMlPrivUsbWaitClockT1xx(void)
{
...
static void NvMlPrivInitBaseAddressT1xx(NvBootUsbfContext * pUsbContext)
{
...
static void NvMlPrivGetPortSpeedT1xx(NvBootUsbfContext * pUsbContext)
{
...
#define NVML_FUN_SOC(FN,ARG) FN##T1xx ARG
#define NVBOOT_FUN_SOC(FN,ARG) FN##T1xx ARG
#include "nvml_usbf.c"
#undef NVML_FUN_SOC

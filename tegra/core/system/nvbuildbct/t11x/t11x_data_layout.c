...
static void BeginUpdate(BuildBctContext *Context, UpdateEndState EndState)
{
    ...
    if (Bct->NumParamSets== 0)
    {
        ...
        else if (Context->SecondaryBootDevice == Nv3pDeviceType_Usb3)
        {
            Bct->DevType[0] = NvBootDevType_Usb3;
            Bct->DevParams[0].Usb3Params.ClockDivider = 0;
            Bct->DevParams[0].Usb3Params.RootPortNumber = 1;
            Bct->DevParams[0].Usb3Params.PageSize2kor16k = 0;
            Bct->DevParams[0].Usb3Params.OCPin = 5;
            Bct->DevParams[0].Usb3Params.VBusEnable = 1;
            Bct->NumParamSets=1;
        }
        else
        {
...
void t11xUpdateBct(BuildBctContext *Context, UpdateEndState EndState)
{
    BeginUpdate(Context, EndState);
    FinishUpdate(Context, EndState);
}

void t11xUpdateBl(BuildBctContext *Context, UpdateEndState EndState)
{
    FillBootloaderData(Context, EndState);
}

    ...
    NvU32 PartitionId)
{
    ...
    NvPartInfo PTInfo;
    NvU32 curBlock = 0, curBit = 0, curIndex = 0, partitionSize = 0, maxBlock = 0;
    NvU32 bootromBlockSize, devBlockSize, i;
    ...
    // No bad block table for NOR
    if((NvDdkBlockDevMgrDeviceId)minfo.DeviceId != NvDdkBlockDevMgrDeviceId_Nand)
    {
        e = NvSuccess;
        goto fail;
    }
    ...
    while (curBlock < maxBlock)
    {
        ...
        badBlockTable.BadBlocks[curIndex] |= (!query_arg_out.IsGoodBlock << curBit);

...
/*
 * Original work done by Karl Malbrain malbrain@yahoo.com
 *
...
static void ShiftRows       (NvU8 *state);
static void InvShiftRows    (NvU8 *state);
static void MixSubColumns   (NvU8 *state);
static void InvMixSubColumns(NvU8 *state);
static void AddRoundKey     (NvU32 *state, NvU32 *key);
...
MixSubColumns(NvU8 *state)
{
    ...
    NvOsMemcpy (state, tmp, sizeof(tmp));
}

// restore and un-mix each row in a column
void InvMixSubColumns (NvU8 *state)
{
    NvU8 tmp[4 * NVAES_STATECOLS];
    int  i;
...
void
AddRoundKey(NvU32 *state, NvU32 *key)
{
    int idx;
...
void
NvAesExpandKey(NvU8 *key, NvU8 *expkey)
{
    NvU8  tmp0, tmp1, tmp2, tmp3, tmp4;
    NvU32 idx;

    NvOsMemcpy(expkey, key, NVAES_KEYCOLS * 4);
...
void
NvAesEncrypt(NvU8 *in, NvU8 *expkey, NvU8 *out)
{
    NvU8  state[NVAES_STATECOLS * 4];
    NvU32 round;

    NvOsMemcpy(state, in, NVAES_STATECOLS * 4);
    AddRoundKey((NvU32 *)state, (NvU32 *)expkey);
    ...
    for (round = 1; round < NVAES_ROUNDS + 1; round++)
    {
        ...
        AddRoundKey((NvU32 *)state, (NvU32 *)expkey + round * NVAES_STATECOLS);
    }

    NvOsMemcpy(out, state, sizeof(state));
}

void
NvAesDecrypt(NvU8 *in, NvU8 *expkey, NvU8 *out)
{
    NvU8  state[NVAES_STATECOLS * 4];
    NvU32 round;

    NvOsMemcpy(state, in, sizeof(state));

    AddRoundKey((NvU32 *)state,

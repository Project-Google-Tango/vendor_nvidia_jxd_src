                      ...
                      size_t *SrcSize)
{
    ...
    *SrcSize = *SrcSize - 2;
    ...
    if ((*SrcSize>1) && ((tmp & 0xFC00UL) == 0xd800UL))
    {
...
//===========================================================================
// NvOsShowError() - show an error and return it (do nothing if err==NvSuccess)
//===========================================================================
NvError
NvOsShowError(NvError err, const char *file, int line)
{
    if (err)
    {
        NvOsDebugPrintf("Error 0x%08x occurred at %s:%d\n",err,file,line);
    }
    return err;
}

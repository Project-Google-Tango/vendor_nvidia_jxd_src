        ...
        NvDdkUsbPhy *pUsbPhy)
{
    ...
    RegVal = USB_IF_REG_RD(SUSP_CTRL);
    RegVal = USB3_IF_FLD_SET_DRF_DEF(SUSP_CTRL, UTMIP_PHY_ENB, ENABLE, RegVal);
    USB_IF_REG_WR(SUSP_CTRL, RegVal);

    USB_UTMIP_REG_UPDATE_NUM(TX_CFG0, UTMIP_FS_PREAMBLE_J, 0x1);
    USB_REG_UPDATE_DEF(USBMODE, CM, HOST_MODE);

    RegVal = NV_READ32(CarBaseAddr+
    ...
    RegVal = USB_UTMIP_REG_RD(BIAS_CFG0);
    ...
    RegVal = NV_FLD_SET_DRF_NUM(USB1_UTMIP,
                    ...
                    0x2,
                    RegVal);
    USB_UTMIP_REG_WR(BIAS_CFG0, RegVal);
    ...
    RegVal = NV_FLD_SET_DRF_NUM(USB1_UTMIP,
                   ...
                   s_UsbBiasTrkLengthTime[FreqIndex],
                   RegVal);
        ...
        NvBool Enable)
{
    ...
    if (Enable)
    {
        ...
        if (pUsbPhy->Instance != 0)
        {
            ...
            RegVal = NV_FLD_SET_DRF_DEF(CLK_RST_CONTROLLER, RST_DEVICES_L, SWR_USBD_RST, ENABLE, RegVal);

            NV_WRITE32(CarBaseAddr + CLK_RST_CONTROLLER_RST_DEVICES_L_0, RegVal);

            NvOsWaitUS(2);

            RegVal =     NV_READ32(CarBaseAddr + CLK_RST_CONTROLLER_RST_DEVICES_H_0);
        ...
        if (!pUsbPhy->pUtmiPadConfig->PadOnRefCount)
        {
            /* UTMI PAD control logic is common to all UTMIP phys and
               they are controlled from USB2 controller */
            // Power Up OTG and Bias config pad circuitry
            RegVal = NV_READ32(pUsbPhy->pUtmiPadConfig->pVirAdr + ((USB2_UTMIP_BIAS_CFG0_0)/4));
            ...
            NV_WRITE32(pUsbPhy->pUtmiPadConfig->pVirAdr + ((USB2_UTMIP_BIAS_CFG0_0)/4),
                    RegVal);
            NvOsWaitUS(1);
    ...
    USB_IF_REG_UPDATE_DEF(SUSP_CTRL, UHSIC_RESET, ENABLE);
        ...
        USB_REG_WR(TXFILLTUNING, 0x00100000);
        RegVal = NV_READ32(0x7D004498);
        RegVal |= USB_HS_RSM_EOP_EN;
        RegVal |= USB_PORT_SUSPEND_EN;
        NV_WRITE32(0x7D004498, RegVal);
        NvOsWaitUS(100);
...
//    dump_address(0x60006000, 0x0fff);
}

static void
T124UsbPhySelectUsbMode(
        NvDdkUsbPhy *pUsbPhy)
{
    NvU32 RegVal = 0;
    ...
    if (pUsbPhy->pProperty->UsbInterfaceType == NvOdmUsbInterfaceType_Utmi)
    {
        ...
        RegVal = USB_FLD_SET_DRF_DEF(HOSTPC1_DEVLC, STS, PARALLEL_IF, RegVal);
                USB_REG_WR(HOSTPC1_DEVLC, RegVal);
        RegVal = NV_READ32(0x7009F00C);
        RegVal = RegVal & (~0x38);
        RegVal = RegVal | (0x20);
        NV_WRITE32(0x7009F00C, RegVal);
        RegVal= USB_REG_READ_VAL(USBMODE, CM);
...
static void
T124UsbPhyUlpiNullModeConfigure(
        NvDdkUsbPhy *pUsbPhy)
{
...
static void
T124UsbPhyUlpiLinkModeConfigure(
        ...
        const void *pInputArgs)
{
    ...
    if (pUsbPhy->pProperty->UsbInterfaceType == NvOdmUsbInterfaceType_Utmi)
    {
        ...
        RegVal = USB2_IF_REG_RD(USB_PHY_VBUS_SENSORS);
        if (pVbusIntr->EnableVBusInterrupt)
        {
            // VBUS_SENSORS -- A_SESS_VLD_INT_EN
            RegVal = NV_FLD_SET_DRF_DEF(USB2_IF, USB_PHY_VBUS_SENSORS,
        ...
        USB2_IF_REG_WR(USB_PHY_VBUS_SENSORS, RegVal);
#else
...
#ifdef NVODM_BOARD_IS_FPGA
        RegVal = USB2_IF_REG_RD(USB_PHY_VBUS_SENSORS);
        if (NV_DRF_VAL(USB2_IF, USB_PHY_VBUS_SENSORS, A_SESS_VLD_STS, RegVal))
#else
            RegVal = USB1_IF_REG_RD(USB_PHY_VBUS_WAKEUP_ID);
        ...
        const void *pInputArgs)
{
    ...
    if (pUsbPhy->pProperty->UsbInterfaceType == NvOdmUsbInterfaceType_Utmi)
    {
        ...
        {
            RegVal = NV_FLD_SET_DRF_DEF(USB2_IF, USB_PHY_VBUS_WAKEUP_ID,
                    ID_PU, ENABLE, RegVal);
            RegVal = NV_FLD_SET_DRF_DEF(USB2_IF, USB_PHY_VBUS_WAKEUP_ID,
...
static NvError
T124UsbPhyIoctlDedicatedChargerDetection(
        NvDdkUsbPhy *pUsbPhy,
        const void *pInputArgs)
{
...
#define TDPSRC_CON_MS    40
    ...
    if (pChargerDetection->EnableChargerDetection)
    {
        ...
        RegVal = USB_UTMIP_REG_RD(BAT_CHRG_CFG0);
        RegVal = NV_FLD_SET_DRF_NUM(USB2_UTMIP, BAT_CHRG_CFG0,
...
static NvError
T124UsbPhyIoctlDedicatedChargerStatus(
        NvDdkUsbPhy *pUsbPhy,
        void *pOutputArgs)
{
        ...
        void *pOutputArgs)
{
    ...
    switch (IoctlType)
    {
        case NvDdkUsbPhyIoctlType_VBusStatus:
            ErrStatus = T124UsbPhyIoctlVBusStatus(pUsbPhy, pOutputArgs);
            break;
        case NvDdkUsbPhyIoctlType_VBusInterrupt:
            ErrStatus = T124UsbPhyIoctlVbusInterrupt(pUsbPhy, pInputArgs);
            break;
        case NvDdkUsbPhyIoctlType_IdPinStatus:
            ErrStatus = T124UsbPhyIoctlIdPinStatus(pUsbPhy, pOutputArgs);
            break;
        case NvDdkUsbPhyIoctlType_IdPinInterrupt:
            ErrStatus = T124UsbPhyIoctlIdPinInterrupt(pUsbPhy, pInputArgs);
            break;
        case NvDdkUsbPhyIoctlType_DedicatedChargerStatus:
            ErrStatus = T124UsbPhyIoctlDedicatedChargerStatus(pUsbPhy, pOutputArgs);
            break;
        case NvDdkUsbPhyIoctlType_DedicatedChargerDetection:
            ErrStatus = T124UsbPhyIoctlDedicatedChargerDetection(pUsbPhy, pInputArgs);
            break;
        default:
            return NvError_NotSupported;
    }
        ...
        NvDdkUsbPhy *pUsbPhy)
{
    ...
    do {
        ...
        if (!TimeOut)
        {
            NvOsDebugPrintf(" ERROR:: PHY CLK not VALID line: %d %s\n", __LINE__, __func__);
        ...
        NvDdkUsbPhy *pUsbPhy)
{
    ...
    switch (pUsbPhy->pProperty->UsbInterfaceType)
    {
        case NvOdmUsbInterfaceType_UlpiNullPhy:
            T124UsbPhyUlpiNullModeConfigure(pUsbPhy);
            T124UsbPhyUlpiPowerControl(pUsbPhy, NV_TRUE);
            break;
        case NvOdmUsbInterfaceType_UlpiExternalPhy:
            pUsbPhy->hOdmUlpi = NvOdmUsbUlpiOpen(pUsbPhy->Instance);
            T124UsbPhyUlpiLinkModeConfigure(pUsbPhy, NV_TRUE);
            T124UsbPhyUlpiPowerControl(pUsbPhy, NV_TRUE);

            break;
        case NvOdmUsbInterfaceType_Utmi:
            ErrVal = T124UsbPhyUtmiConfigure(pUsbPhy);
            if (ErrVal != NvSuccess)
                return ErrVal;
            T124UsbPhyUtmiPowerControl(pUsbPhy, NV_TRUE);
        default:
            break;
    }
    ErrVal = T124UsbPhyWaitForStableClock(pUsbPhy);
    if (ErrVal == NvSuccess)
    {
        T124UsbPhySelectUsbMode(pUsbPhy);
        ...
        NvDdkUsbPhy *pUsbPhy)
{
    switch (pUsbPhy->pProperty->UsbInterfaceType)
    {
        case NvOdmUsbInterfaceType_UlpiNullPhy:
            break;
        case NvOdmUsbInterfaceType_UlpiExternalPhy:
            T124UsbPhyUlpiLinkModeConfigure(pUsbPhy, NV_FALSE);
            T124UsbPhyUlpiPowerControl(pUsbPhy, NV_FALSE);
            if (pUsbPhy->hOdmUlpi)
            {
            ...
            break;
        case NvOdmUsbInterfaceType_Utmi:
            T124UsbPhyUtmiPowerControl(pUsbPhy, NV_FALSE);
        default:
            break;
    }
        ...
        NvDdkUsbPhy *pUsbPhy)
{
    ...
    NV_ASSERT_SUCCESS(T124UsbPhyPowerDown(pUsbPhy));
}

void
T124UsbPhyOpenHwInterface(
        NvDdkUsbPhy *pUsbPhy)
{
    pUsbPhy->PowerUp = T124UsbPhyPowerUp;
    pUsbPhy->PowerDown = T124UsbPhyPowerDown;
    pUsbPhy->Ioctl = T124UsbPhyIoctl;
    pUsbPhy->WaitForStableClock = T124UsbPhyWaitForStableClock;
    pUsbPhy->CloseHwInterface = T124UsbPhyClose;
}

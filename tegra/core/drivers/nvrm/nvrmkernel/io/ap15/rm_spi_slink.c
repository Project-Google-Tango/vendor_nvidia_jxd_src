    ...
    NvU32 BufferSize)
{
    ...
    *phNewMemHandle = NULL;
    ...
    Error = NvRmMemHandleAlloc(hDevice, NULL,
                0, DMA_BUFFER_ALIGNMENT, NvOsMemAttribute_Uncached,
    ...
    if (Error == NvError_Timeout)
    {
        ...
        if (hRmSpiSlink->IsUsingApbDma)
        {
            if (hRmSpiSlink->CurrentDirection & SerialHwDataFlow_Rx)
            {
                ...
                DmaErr = NvRmDmaGetTransferredCount(hRmSpiSlink->hRmRxDma,
                            &DmaRxTransferCountBytes, NV_TRUE);
                NV_ASSERT(DmaErr == NvSuccess);
                if (DmaErr != NvSuccess)
        ...
        else
        {
            ...
            // avaiable in the fifo. -- Supported on later soc.
            if (hRmSpiSlink->CurrentDirection & SerialHwDataFlow_Rx)
            {
                if ((hRmSpiSlink->CurrTransInfo.PacketsPerWord > 1) &&
                    (!hRmSpiSlink->HwRegs.IsSlaveAllRxPacketReadable))
                        CurrentSlinkPacketTransfer -=
                                    CurrentSlinkPacketTransfer%
                                                hRmSpiSlink->CurrTransInfo.PacketsPerWord;
            }
...
static void DestroySpiSlinkChannelHandle(NvRmSpiHandle hRmSpiSlink)
{
    ...
    NvOsPhysicalMemUnmap(hRmSpiSlink->HwRegs.pRegsBaseAdd, hRmSpiSlink->HwRegs.RegBankSize);
...
{
    static SlinkCapabilities s_SpiCap[3];
    SlinkCapabilities *pSpiCap = NULL;
    static NvRmModuleCapability s_SpiCapList[] =
        {
            {1, 0, 0, NvRmModulePlatform_Silicon, &s_SpiCap[0]}, // AP15 version 1.0
            {1, 1, 0, NvRmModulePlatform_Silicon, &s_SpiCap[1]}, // AP20 version 1.1
            {1, 2, 0, NvRmModulePlatform_Silicon, &s_SpiCap[2]}, // T30  version 1.2
        };
    ...
    // (T30) version 1.2
    s_SpiCap[2].MajorVersion = 1;
    s_SpiCap[2].MinorVersion = 2;
    ...
    if (pSpiCap->MajorVersion != 1)
    {
        NV_ASSERT(0);
    }

    if (pSpiCap->MinorVersion > 0) // Ap20 and above
        NvRmPrivSpiSlinkInitSlinkInterface_v1_x(&s_SlinkHwInterface,
                                                    pSpiCap->MinorVersion);
    if (pSpiCap->MinorVersion == 2) // T30 and above
        NvRmPrivSpiSlinkInitSlinkInterface_v1_2(&s_SlinkHwInterface,
                                                    pSpiCap->MinorVersion);
}

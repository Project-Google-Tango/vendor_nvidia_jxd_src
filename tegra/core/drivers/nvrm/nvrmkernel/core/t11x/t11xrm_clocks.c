...
static const NvU32 s_T11xBondOutTable[] =
{
    // BOND_OUT_L bits
    ...
    ModuleClockState ClockState)
{
    ...
    switch ( Module ) {
        case NvRmModuleID_CacheMemCtrl:
            ...
            if( Instance == 0 )
            {
                NV_ASSERT(!"T11x doesn't have such device");
            }
            else if( Instance == 1 )
            {
...
SetDsibClkToPllD(NvRmDeviceHandle hRmDevice)
{
    ...
    Reg = NV_FLD_SET_DRF_NUM(CLK_RST_CONTROLLER, PLLD2_BASE,
                    DSIB_CLK_SRC, 0, Reg);
    NV_REGW(hRmDevice, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0),
                CLK_RST_CONTROLLER_PLLD2_BASE_0, Reg);
}
...
NvRmBasicReset( NvRmDeviceHandle rm )
{
    ...
    env = NvRmPrivGetExecPlatform(rm);
    {
         ...
         * and VDE - set on PLLP_OUT0/(1+2/2)
         */
        reg = NV_REGR(rm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
        ...
        NVRM_CONFIG_CLOCK(VDE, PLLP_OUT0, 2);
        // Prevent SOC_THERM from overclocking with PLLP @408 MHZ
        NVRM_CONFIG_CLOCK(SOC_THERM, pllP_out0, 4);

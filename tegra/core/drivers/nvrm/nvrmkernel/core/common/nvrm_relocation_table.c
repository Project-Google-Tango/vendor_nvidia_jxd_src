...
#define IRQ_DEVICE_INDEX( i )   ( ( (i) & ( 0x1FFUL   << 8  ) ) >> 8  )
...
NvRmPrivDevToModuleID(NvU32 devid)
{
    ...
    case NVRM_DEVID_MSENC:
        return NvRmModuleID_MSENC;
    case NVRM_DEVID_TSEC:
        return NvRmModuleID_TSEC;
    case NVRM_DEVID_XUSB_HOST:
        return NvRmModuleID_XUSB_HOST;
    ...
    case NVRM_DEVID_HDA:
        return NvRmModuleID_HDA;
    case NVRM_DEVID_APBIF:
        return NvRmModuleID_Apbif;
    case NVRM_DEVID_AUDIO:
        return NvRmModuleID_Audio;
    case  NVRM_DEVID_DAM:
        return NvRmModuleID_DAM;
    case NVRM_DEVID_SATA:
        return NvRmModuleID_Sata;
    case NVRM_DEVID_DTV:
        return NvRmModuleID_DTV;
    case NVRM_DEVID_TSENSOR:
        return NvRmModuleID_Tsensor;
    case NVRM_DEVID_ATOMICS:
        return NvRmModuleID_Atomics;
    case NVRM_DEVID_ACTMON:
        return NvRmModuleID_Actmon;
    case NVRM_DEVID_SE:
        return NvRmModuleID_Se;
    case NVRM_DEVID_EXTERNAL_CLOCK:
        return NvRmModuleID_ExtPeriphClk;
    case NVRM_DEVID_EMEMIO:
        return NvRmPrivModuleID_ExternalMemory_MMIO;
    case NVRM_DEVID_DVFS:
        return NvRmModuleID_ClDvfs;
    ...
    NvRmModule *modules )
{
    ...
    *instances = inst;
    devidx = (NvU16)-1;      /* -1 is the invalid/unavailable indicator */
    ...
    while( NV_READ32( &table[index] ) )
    {
        ...
        {
            /* keep going */
            NV_DEBUG_PRINTF(( "[Unknown] devidx: %d devid: %d start: 0x%x "
                "length: 0x%x\n", devidx, devid, start, length ));
            continue;
        }
        else if( modid == NVRM_DEVICE_ERROR )
        {
            NV_DEBUG_PRINTF(( "[Error] devidx: %d devid: %d start: 0x%x "
                "length: 0x%x\n", devidx, devid, start, length ));
...
NvRmPrivGetInstance( NvRmModuleInstance *inst, NvU16 devidx )
{
    while( inst->DeviceId )
    {
    ...
    NvRmModuleInstance *instances )
{
    ...
    while( NV_READ32( &table[index] ) )
    {
        ...
        devidx      = (NvU16)IRQ_DEVICE_INDEX(info);
        ...
        if (Valid)
        {
            ...
            ||  (Affinity == Processor))
            {
                ...
                NV_DEBUG_PRINTF(("[Interrupt %d] Device Index: %d "
    ...
    NvRmModule *modules, NvRmIrqMap *irqs )
{
    ...
    NV_DEBUG_PRINTF(( "Relocation Table:\n" ));

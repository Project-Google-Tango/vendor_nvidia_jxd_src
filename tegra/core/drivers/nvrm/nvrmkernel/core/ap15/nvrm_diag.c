...
#include "project_relocation_table.h"
...
static const NvU32 s_Ap20PowerRailsTableSize =
    NV_ARRAY_SIZE(s_Ap20PowerRailsTable);
...
NvRmDiagEnable(NvRmDeviceHandle hRmDevice)
{
    ...
    {
        case 0x35: /* FIXME: Use AP20 for now */
        case 0x14: /* FIXME: Use AP20 for now */
        case 0x30: /* FIXME: Use AP20 for now */
            s_Modules.ModuleClockTableSize = g_ModuleClockTableSize;
            s_Modules.ModuleClockTable = g_ModuleClockTable;
    ...
    NvRmDiagClockSourceHandle* phSourceList)
{
    ...
    pCinfo =
        s_Modules.pInstancePtrs[s_Modules.InstancesMap[Module].BaseIndex +
                Instance];
    ...
    NvBool Source1st)
{
    ...
    */
    ...
    if (Source1st)
    {
        ...
        NV_REGW(s_hDiagRm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
            offset, reg);
...
/*
 * Gets power group for the specified module if it is one of system modules,
 * not present in the relocation table. Otherwise, returns
 * NV_POWERGROUP_INVALID.
 */
static NvU32
    ...
    NvRmDiagPowerRailHandle* phRailList)
{
    ...
    for (i = 0; i < s_Rails.PowerRailsNum; i++)
    {
        ...
        while ((*pPowerGroup) != NV_POWERGROUP_INVALID)
        {
            ...
            pPowerGroup++;
            NV_ASSERT(pPowerGroup <
                (s_Rails.PowerRailsTable[i].PowerRailGroups +
    ...
    NvU32 VoltageMV)
{
    ...
    NV_DEBUG_PRINTF(("Setting PMU rail %2d to %5dmV\n", RailAddress,
        VoltageMV));

...
static const NvU32 s_T12xBondOutTable[] =
{
    // BOND_OUT_L bits
    NVRM_DEVICE_UNKNOWN,  // NV_DEVID_CPU
    NVRM_DEVICE_UNKNOWN,  // NV_DEVID_ISPB
    ...
    NVRM_MODULE_ID( NvRmModuleID_Spi,                           1 ),
    NVRM_MODULE_ID( NvRmModuleID_Xio,                           0 ),
    NVRM_MODULE_ID( NvRmModuleID_Spi,                           2 ),
    NVRM_MODULE_ID( NvRmModuleID_I2c,                           4 ),
    ...
    NVRM_MODULE_ID( NvRmModuleID_Spi,                           3 ),
    ...
    NVRM_DEVICE_UNKNOWN,    // TRACECLKIN
    NVRM_DEVICE_UNKNOWN,    // SOC_THERM
    NVRM_MODULE_ID( NvRmModuleID_DTV,                           0 ),
    NVRM_DEVICE_UNKNOWN,    // I2C_SLOW
    NVRM_MODULE_ID( NvRmModuleID_Dsi,                           1 ),
    NVRM_MODULE_ID( NvRmModuleID_TSEC,                          0 ),// TSEC
    ...
    NVRM_MODULE_ID( NvRmModuleID_XUSB_HOST,                     0 ),// TSEC
    NVRM_MODULE_ID( NvRmModuleID_MSENC,                         0 ),// MSENC
    NVRM_DEVICE_UNKNOWN,    // SUS_OUT
    NVRM_DEVICE_UNKNOWN,    // DEV2_OUT
    NVRM_DEVICE_UNKNOWN,    // DEV1_OUT
    NVRM_DEVICE_UNKNOWN,    // XUSB_DEV
    ...
    NVRM_MODULE_ID( NvRmModuleID_Spi,                           4 ),
    NVRM_MODULE_ID( NvRmModuleID_Spi,                           5 ),
    ...
    NVRM_DEVICE_UNKNOWN,    // EXTPERIPH1
    NVRM_DEVICE_UNKNOWN,    // EXTPERIPH2
    NVRM_DEVICE_UNKNOWN,    // EXTPERIPH3
...
NvRmGetBondOut( NvRmDeviceHandle hDevice,
                       ...
                       NvU32           *bondOut )
{
    *pTable = s_T12xBondOutTable;
    bondOut[0] = NV_REGR(hDevice,
        NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
        CLK_RST_CONTROLLER_BOND_OUT_L_0);
#if UPDATE_DSI_BONDOUT
    bondOut[0] = bondOut[0] & (!(3 << 26)); //FIX IT BUG 920412:DSI is bondout on 7.2
#endif
    ...
    bondOut[5] = NV_REGR(hDevice,
        NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
        CLK_RST_CONTROLLER_BOND_OUT_X_0);
}
    ...
    ModuleClockState ClockState)
{
    ...
    switch ( Module ) {
        case NvRmModuleID_CacheMemCtrl:
            ...
            if( Instance == 0 )
            {
                NV_ASSERT(!"T12x doesn't have such device");
            }
            else if( Instance == 1 )
            {
        ...
        case NvRmModuleID_I2c:
            ...
            case 4:
                ...
                break;
            case 5:
                CLOCK_ENABLE( hDevice, X, I2C6, ClockState );
                break;
            default:
        ...
        case NvRmModuleID_MSENC:
            NV_ASSERT( Instance == 0 );
            CLOCK_ENABLE( hDevice, U, MSENC, ClockState );
            break;
        case NvRmModuleID_TSEC:
            NV_ASSERT( Instance == 0 );
            CLOCK_ENABLE( hDevice, U, TSEC, ClockState );
            break;
        ...
        case NvRmModuleID_Slink:
            ...
            {
                CLOCK_ENABLE( hDevice, H, SPI2, ClockState );
            }
            else if ( Instance == 2)
            {
                CLOCK_ENABLE( hDevice, H, SPI3, ClockState );
            }
            else if ( Instance == 3)
            {
                CLOCK_ENABLE( hDevice, U, SPI4, ClockState );
            }
            else if ( Instance == 4)
            {
                CLOCK_ENABLE( hDevice, V, SPI5, ClockState );
            }
            else if ( Instance == 5)
            {
                CLOCK_ENABLE( hDevice, V, SPI6, ClockState );
            }
            break;
        ...
        case NvRmModuleID_Se:
            ...
            break;
        case NvRmModuleID_ClDvfs:
            NV_ASSERT( Instance == 0 );
            CLOCK_ENABLE( hDevice, W, DVFS, ClockState );
            break;
        default:
            NV_ASSERT(!"Unknown NvRmModuleID passed to T12xEnableModuleClock().");
    }
...
void ModuleReset(NvRmDeviceHandle hDevice, NvRmModuleID ModuleId, NvBool hold)
{
    ...
    case NvRmModuleID_Pmif:
        NV_ASSERT( Instance == 0 );
        NV_ASSERT(!"PMC reset is not allowed, and does nothing on T124");
        break;
    case NvRmModuleID_Fuse:
        NV_ASSERT( Instance == 0 );
    ...
    case NvRmModuleID_Slink:
        ...
        {
            RESET( hDevice, H, SPI2, NVRM_RESET_DELAY );
        }
        else if (Instance == 2)
        {
            RESET( hDevice, H, SPI3, NVRM_RESET_DELAY );
        }
        else if (Instance == 3)
        {
            RESET( hDevice, U, SPI4, NVRM_RESET_DELAY );
        }
        else if ( Instance == 4)
        {
            RESET( hDevice, V, SPI5, NVRM_RESET_DELAY );
        }
        else if ( Instance == 5)
        {
            RESET( hDevice, V, SPI6, NVRM_RESET_DELAY );
        }
        break;
    case NvRmModuleID_Xio:
        NV_ASSERT( Instance == 0 );
    ...
    case NvRmModuleID_Dsi:
        ...
        if( Instance == 1)
            RESET( hDevice, U, DSIB, NVRM_RESET_DELAY );
        break;
    case NvRmModuleID_Mipi:
        NV_ASSERT( Instance == 0 );
    ...
    case NvRmModuleID_I2c:
        ...
        case 4:
            ...
            break;
        case 5:
            RESET( hDevice, X, I2C6, NVRM_RESET_DELAY );
            break;
        default:
    ...
    case NvRmModuleID_MSENC:
        NV_ASSERT( Instance == 0 );
        RESET( hDevice, U, MSENC, NVRM_RESET_DELAY );
        break;
    case NvRmModuleID_TSEC:
        NV_ASSERT( Instance == 0 );
        RESET( hDevice, U, TSEC, NVRM_RESET_DELAY );
        break;
    case NvRmModuleID_Vde:
        NV_ASSERT( Instance == 0 );
        {
...
{
    // Not necessary for PLLD
}

// Safe PLLM (max 1067MHz) divider for GPU modules
#define NVRM_SAFE_GPU_DIVIDER ((10 - 1) << 1)
...
{
    NvU32 reg, ClkOutL, ClkOutH, ClkOutU, ClkOutV, ClkOutW, ClkOutX;
    ExecPlatform env;
    ...
    ClkOutX = NV_REGR(rm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
    ...
    NV_REGW( rm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
       ...
       CLK_RST_CONTROLLER_CLK_ENB_X_SET_0_WRITE_MASK );
    ...
    env = NvRmPrivGetExecPlatform(rm);
    {
        ...
        if (reg & CLK_RST_CONTROLLER_RST_DEVICES_L_0_SWR_SPDIF_RST_FIELD)
        {
            reg = NV_DRF_DEF(CLK_RST_CONTROLLER, CLK_SOURCE_SPDIF_IN,
                             SPDIF_IN_CLK_SRC, PLLP_OUT0) |
                  NV_DRF_NUM(CLK_RST_CONTROLLER, CLK_SOURCE_SPDIF_IN,
                             SPDIF_IN_CLK_DIVISOR, 16);
            NV_REGW( rm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
                CLK_RST_CONTROLLER_CLK_SOURCE_SPDIF_IN_0, reg);
        }
        NVRM_CONFIG_CLOCK(VDE, PLLP_OUT0, 2);           // PLLP_OUT0/(1+2/2)
        NVRM_CONFIG_CLOCK(SOC_THERM, PLLP_OUT0, 4);     // PLLP_OUT0/(1+4/2)
        NVRM_CONFIG_CLOCK(HDMI_AUDIO, PLLP_OUT0, 18);   // PLLP_OUT0/(1+18/2)
        ...
        if (env != ExecPlatform_Sim)
            NVRM_CONFIG_CLOCK(MSENC, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(TSEC, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(VI, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(VI_SENSOR, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(VI_SENSOR2, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(ISP, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
        NVRM_CONFIG_CLOCK(VIC, PLLM_OUT0, NVRM_SAFE_GPU_DIVIDER);
    }
    ...
    NV_REGW( rm, NVRM_MODULE_ID( NvRmPrivModuleID_ClockAndReset, 0 ),
        CLK_RST_CONTROLLER_RST_DEV_X_CLR_0,
        CLK_RST_CONTROLLER_RST_DEV_X_CLR_0_WRITE_MASK );

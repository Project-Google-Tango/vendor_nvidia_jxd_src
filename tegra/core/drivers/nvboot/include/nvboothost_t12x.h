 ...
 * 2. call NvBootHostT12xRcmGetMsgBufferLength() to determine the buffer size
 *    needed to hold the message
 * 3. allocate a buffer to hold the message
 * 4. call NvBootHostT12xRcmInitMsg() to initialize the message
 * 5. call NvBootHostT12xRcmGetMsgPayloadBuffer() to get a pointer to the payload
 *    buffer within the message
 * 6. copy the payload data into the payload buffer within the message
 * 7. call NvBootHostT12xRcmEncryptSignMsg() to sign and (optionally) encrypt
 *    the message
 * 8. call NvBootHostT12xRcmGetMsgLength() to get the length of the message
...
#ifndef INCLUDED_NVBOOTHOST_T12X_H
#define INCLUDED_NVBOOTHOST_T12X_H

#include "nvos.h"
#include "nverror.h"
#include "t12x/nvboot_se_rsa.h"
#include "t12x/nvboot_hash.h"
...
NvU32
NvBootHostT12xRcmGetMsgBufferLength(NvU32 PayloadLength);
...
NvError
NvBootHostT12xRcmSignMsg(NvU8 *MsgBuffer,
 ...
 *
 * Note: the routine should only be called after NvBootHostT12xRcmInitMsg()
 *
 * @param MsgBuffer buffer where message is stored
 *
...
#endif // INCLUDED_NVBOOTHOST_T12X_H

...
NvU32
NvBootHostRcmGetMsgBufferLength(
    NvU32 PayloadLength)
{
    ...
    NvU32 PayloadLength)
{
    ...
    NvOsMemset(&(Msg->Hash), 0x0, sizeof(Msg->Hash));
    ...
    NvOsMemset((NvU8 *)&(Msg->Args) + ArgsLength, 0x0, sizeof(Msg->Args) - ArgsLength);
    ...
    NvU8 *Sbk)
{
    ...
    CryptoLength = Msg->LengthInsecure-sizeof(NvU32)-sizeof(NvBootHash);
    ...
    NvU8 **pMsgBuffer)
{
    ...
    MsgBufferLength = NvBootHostRcmGetMsgBufferLength(PayloadLength);
    ...
    NvBootHostRcmInitMsg(MsgBuffer, MsgBufferLength, OpCode, RandomAesBlock, 
                         Args, ArgsLength, PayloadLength);
    
    // fill message payload

    MsgPayloadBuffer = NvBootHostRcmGetMsgPayloadBuffer(MsgBuffer);
    ...
    e = NvBootHostRcmSignMsg(MsgBuffer, OpMode, Sbk);
    if ( e != NvSuccess )
        goto done;
    ...
    NvU8 **pMsgBuffer)
{
    ...
    PayloadBuffer = NvBootHostRcmGetMsgPayloadBuffer(MsgBuffer);
    ...
    NvU8 **pMsgBuffer)
{
    ...
    *pMsgBuffer = (NvU8 *)NULL;
    ...
    e = NvBootHostRcmCreateMsgFromFileHandle(OpCode, RandomAesBlock, Args,

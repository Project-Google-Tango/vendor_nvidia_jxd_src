...
/**
 * nvboot_usb3_context.h - Definitions for the Usb3
 * context structure.
 */

#ifndef INCLUDED_NVBOOT_USB3_CONTEXT_H
#define INCLUDED_NVBOOT_USB3_CONTEXT_H
...
typedef enum
{
    NvBootXusbStatus_WorkQSubmitted = 1,
    NvBootXusbStatus_CompQPollStart,
    NvBootXusbStatus_CompQPollError,
    NvBootXusbStatus_CompQPollEnd,
    NvBootXusbStatus_EventQPollStart,
    NvBootXusbStatus_EventQPollError,
    NvBootXusbStatus_EventQPollEnd,

}NvBootXusbStatus;
/*
 * NvBootUsb3Context - The context structure for the Usb3 driver.
...
typedef struct NvBootUsb3ContextRec
{
    ...
    //BufferXusbData space else it is the address of the memory area passed from the top level flow
    NvU8 *BufferData;
    //Required for Read command
    NvU8 LogicalBlkAddr[4];
    NvU8 TransferLen[2];
    ...
    struct
    {
        NvU16 PacketSize;
        NvU8  EndpointNumber;
    }EPNumAndPacketSize[2]; //OUT is indexed by 0 (direction out) and IN type is indexed by 1 (direction in).
    struct
    {
    ...
    /// Holds the Usb3 Read start time.
    NvU64 ReadStartTime;
...
NvBootError NvBootXusbMscBotProcessRequest(NvBootUsb3Context *Context, NvU8 opcode);
...
#endif /* #ifndef INCLUDED_NVBOOT_USB3_CONTEXT_H */

...
// transactions (on going burst) will be completed and no new transactions will be initiated. Setting this bit again resumes the transfers. 
// Note that the power on reset value for the APB DMA Global Enable is 0. This bit must be written to 1 before any APB DMA transactions can 
// begin.
...
// The Busy bits in the Status Register indicate which (if any) of the APB DMA channels have active pending APB DMA transfers. Note that 
// APB DMA transfers that are started in continuous (repetitive) mode will have their busy bits active until the enable bit for the APB DMA 
// channel is cleared to 0 (by the software)
...
// The Tx requestor register is read only. 
// This register is provided only to give the software the ability to see the state of signals that may affect
// (trigger-start or flow-control-inhibit) APB DMA processes. 
// Bit[0] in the Tx Requestor channel is a signal generated from the programmable APB DMA Counter. 
// This bit is active whenever the APB DMA Counter value is 0.
...
// The Rx requestor register is read only. 
// This register is provided only to give the software the ability to see the state of signals that may affect
// (trigger-start or flow-control-inhibit) APB DMA processes. 
// Bit[0] in the Rx Requestor channel is a signal generated from the programmable APB DMA Counter. 
// This bit is active whenever the APB DMA Counter value is 0.
...
// The APB DMA Counter register stores bits that configure which (if any) channel(s) should be throttled (bits[31:16] 
// correspond to APB DMA 16 channels.
// The APB DMA Counter initial/reload count value is programmed in the APB DMA Counter Register (bits[15:0]).The APB DMA Counter is loaded
// with this initial/reload count value whenever the APB DMA Counter Register is written, and is re-loaded to 
// this saved initial count value on a burst complete (programmable). The APB DMA Counter value will decrement whenever an APB DMA burst 
// complete, and the current count value is non-zero, and any of the bits [31:16] are set.

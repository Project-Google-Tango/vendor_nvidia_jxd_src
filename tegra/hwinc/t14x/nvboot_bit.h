...
/**
 * @file
 * <b>NVIDIA Boot Information Table (BIT) for Tegra 4i (T14x) series processors</b>
 *
 */

/**
 * @defgroup nvbdk_bit Boot Information Table APIs for Tegra 4i (T14x) series processors
 *
...
typedef struct NvBootInfoTableRec
{
    ...
    /// - [4:4] = IsDividerChargerDetected (1 if divider charger detected, 0 if not).
    /// - [5:5] = IsACADetected (1 if ACA A/B/C detected, 0 if not).
    /// - [6:6] = IsNonCompliantChargerDetected
    ///           (1 if charger is non-compliant, 0 if not).
    /// - [7:7] = IsDcpCdpDetected (1 if DCP/CDP, 0 if not).
    /// - [4:4] = PmicHighCurrentAsserted (1 if driven high, 0 if low).
    /// - [9:9] = DeviceEnumerationComplete (1 if device enumeration was performed).
    /// - [10:10] = DeviceEnumerationFailed
    ///             (1 if device enum failed, 0 if it succeeded).
    /// - [11:11] = ReadPMICUsbChargerControlForHiCurrentFailed
    ///             (1 if read/write failed, 0 it succeeded).
    /// - [12:12] = UsbChargingBITStat_ReadPMICUsbChargerControlForUsbSuspendFailed
    ///             (1 if read/write failed, 0 if it succeeded).
    /// - [13:13] = UsbChargingBITStat_Timer0Disabled
    ///             (set to 1 if Timer0 disabled, 0 if enabled).
    NvU32 UsbChargingStatus;

    /// NvBootError_Success indicates that reading the BOOT_SEL

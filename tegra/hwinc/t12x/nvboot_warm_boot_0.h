...
typedef struct NvBootWb0RecoveryHeaderRec
{
    ...
    /// Specifies the RSA public key's modulus, needed for verifying
    /// the recovery code. The public key exponent is 0x10001, which
    /// is not included in this header.
    NvBootRsaKeyModulus PublicKey;

    /// Specifies the AES-CMAC MAC or RSASSA-PSS signature computed
    /// over the header and code, starting at RandomAesblock

 ...
 *  * added HAVE_PYTHON_ tests around all python code
 *
...
#define DEFAULT_CONFIG_FILE	"buttons"
#define CONN_STATUS "/data/misc/wminput/connected"

#define HOME_DIR_LEN	128

void set_rpt_mode_flags(struct plugin *plugin,char *name)
{
        if (!strcmp(name,"acc"))
        plugin->rpt_mode_flags = CWIID_RPT_ACC;
        if (!strcmp(name,"ir_ptr"))
        plugin->rpt_mode_flags = CWIID_RPT_IR;
        if (!strcmp(name,"led"))
        plugin->rpt_mode_flags = CWIID_RPT_BTN;
        if (!strcmp(name,"nunchuk_acc"))
        plugin->rpt_mode_flags = CWIID_RPT_STATUS|CWIID_RPT_NUNCHUK;
        if (!strcmp(name,"nunchuk_stick2btn"))
        plugin->rpt_mode_flags = CWIID_RPT_NUNCHUK;
...
int cwiid_connect_status(int status)
{
	int fd=open(CONN_STATUS,O_RDWR);
	if (fd < 0)
		return -1;
	if (status == 1)
		write(fd,"1",1);
...
int main(int argc, char *argv[])
{
	...
	do {
		...
		for (i=0; (i < CONF_MAX_PLUGINS) && conf.plugins[i].name; i++) {
			...
			case PLUGIN_C:
				set_rpt_mode_flags(&conf.plugins[i],conf.plugins[i].name);
		...
		cwiid_connect_status(1);
		if (cwiid_set_led(wiimote, 0x1)) {
...
*/
		cwiid_connect_status(0);
                if ( (status = pthread_kill(uinput_listen_thread, SIGUSR1)) != 0)
                {
                        wminput_err("Error cancelling thread %d, error = %d", uinput_listen_thread, status);
			ret = -1;
                }

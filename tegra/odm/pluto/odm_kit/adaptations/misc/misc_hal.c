            ...
            NvU32 Instance)
{
    ...
    // for SD Card, Pluto PowerRails_VDDIO_SDMMC3 should be powered up
    if(Instance == INSTANCE_SDMMC3 && Module == NvOdmIoModule_Sdio)
    {
        pPerConnectivity = NvOdmPeripheralGetGuid(NV_ODM_GUID('N','V','D','D','S','D','I','O'));
        ...
        if (pPerConnectivity != NULL)
        {
            ...
            for (i = 0; i < pPerConnectivity->NumAddress; i++)
            {
                ...
                if (pPerConnectivity->AddressList[i].Interface == NvOdmIoModule_Vdd)
                {
                    NvOdmServicesPmuGetCapabilities(hPmuTemp,
                                                pPerConnectivity->AddressList[i].Address,
                                                (NvOdmServicesPmuVddRailCapabilities *) &RailCaps);
                    NvOdmServicesPmuSetVoltage(hPmuTemp, pPerConnectivity->AddressList[i].Address,
        ...
        // Enable power rail gpio for sdcard
        if( !hGpio )
        {
            hGpio = NvOdmGpioOpen();
            if( !hGpio )
            ...
            // get gpio GMI_CLK pin for sdmmc3 from odm
            gpioPinTable = NvOdmQueryGpioPinMap(NvOdmGpioPinGroup_Sdio, INSTANCE_SDMMC3,
                                           &gpioPinCount);
            hgmi_clk = NvOdmGpioAcquirePinHandle(hGpio,
                    gpioPinTable[0].Port,
                    gpioPinTable[0].Pin);

            // configure GMI_CLK as output pin
            NvOdmGpioConfig(hGpio, hgmi_clk, NvOdmGpioPinMode_Output);

            // set GMI_CLK pin as high to enable power rail
            NvOdmGpioSetState(hGpio, hgmi_clk, 0x1);
        }
    }

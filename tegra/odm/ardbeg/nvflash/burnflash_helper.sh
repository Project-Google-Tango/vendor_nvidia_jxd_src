...
BoardValidateParams()
{
    ...
    if [ ${FlashQNX} -eq 1 ]; then
        BCT=${NvFlashDir}/PM358_Hynix_2GB_H5TC4G63AFR_RDA_792MHz.cfg
        # FIXME Use prebuilt fastboot binary till tegraflash gets supported.
        BurnFlashBin=${NvFlashDir}/bootloader.bin
    fi
    ...
    let "ODMData = 0x4009C000" # UART-D

    if [ -z $ConfigFile ]; then
        if [ ${FlashQNX} -eq 1 ]; then
            if [ $Quickboot -eq 0 ] ; then
                ConfigFile=${NvFlashDir}/qnx_fastboot_emmc_full.cfg
            else

...
static NvPingroupConfig p1852_pinmux_avp[] = {
    /* UART1 pinmux */
...
#if AVP_PINMUX == 0
static NvPingroupConfig p1852_pinmux_common[] = {
    /* SDMMC1 pinmux */
...
static NvPinDrivePingroupConfig p1852_drive_pinmux[] = {
    /* ATC1 CFG */
...
NvError NvOdmPinmuxInit(NvU32 BoardId)
{
    ...
    err = NvPinmuxConfigTable(p1852_pinmux_avp, NV_ARRAY_SIZE(p1852_pinmux_avp));
    NV_CHECK_PINMUX_ERROR(err);

#if AVP_PINMUX == 0
    err = NvPinmuxConfigTable(p1852_pinmux_common, NV_ARRAY_SIZE(p1852_pinmux_common));
    NV_CHECK_PINMUX_ERROR(err);

    err = NvPinmuxDriveConfigTable(p1852_drive_pinmux,
       NV_ARRAY_SIZE(p1852_drive_pinmux));
    NV_CHECK_PINMUX_ERROR(err);
#endif

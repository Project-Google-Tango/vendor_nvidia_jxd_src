...
static NvU32 s_ProjId;
static NvBool s_bIsDisplayE1188;

#if NVOS_IS_QNX
#include <fcntl.h>
#define NVSKUINFOMAXLEN     30
 ...
 */
static NvU32
NvOdmGetId(char *node)
{
#if NVOS_IS_QNX
    int skuFd = -1;
    char buffer[NVSKUINFOMAXLEN + 1];
    NvU32 Id;

    memset(buffer, 0, NVSKUINFOMAXLEN + 1);
    /* Open the node */
    skuFd = open(node, O_RDONLY);
    if (skuFd == -1)
        return -1;

    /* read the project number. */
    read(skuFd, buffer, NVSKUINFOMAXLEN);
    Id = (NvU32) strtoul(buffer, NULL, 10);
...
#if NVOS_IS_QNX
        if (s_ProjId == 61858 || s_ProjId == 61868) {
...
lcd_wvga_GetHal( NvOdmDispDeviceHandle hDevice )
{
    ...
    s_SkuId = NvOdmGetId("/dev/nvsku/sku");
    s_ProjId = NvOdmGetId("/dev/nvsku/project");
    /*
...
#if NVOS_IS_QNX
    NV_ASSERT((s_ProjId == 61858) || (s_ProjId == 61868));
#endif
}

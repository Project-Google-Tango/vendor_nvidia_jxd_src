...
import com.nvidia.inputmethod.keyboard.internal.SuddenJumpingTouchEventHandler;
...
import com.nvidia.inputmethod.latin.Utils.UsabilityStudyLogUtils;
...
public final class MainKeyboardView extends KeyboardView implements PointerTracker.KeyEventHandler,
    ...
    public boolean moreKeysShowing() {
        return mMoreKeysPanel != null && mMoreKeysWindow.isShowing();
    }
    public Keyboard getMoreKeysKeyboard() {
        if (mMoreKeysPanel != null){
            return ((MoreKeysKeyboardView)mMoreKeysPanel).getKeyboard();
        }
        return null;
    }
    public void invalidateMoreKeysView() {
        ((MoreKeysKeyboardView)mMoreKeysPanel).invalidateAllKeys();
    }
    ...
    private boolean openMoreKeysPanel(final Key parentKey, final PointerTracker tracker) {
        ...
        ((MoreKeysKeyboardView)moreKeysPanel).getKeyboard().selectDefault();
        ((MoreKeysKeyboardView)moreKeysPanel).invalidateAllKeys();
        return true;
    }
    ...
    public boolean processMotionEvent(final MotionEvent me) {
        ...
        if (nonDistinctMultitouch) {
            ...
            if (pointerCount == 1 && oldPointerCount == 2) {
                ...
                if (mOldKey != newKey) {
                    tracker.onDownEvent(x, y, eventTime, this, this.mKeyboard);
                ...
                tracker.processMotionEvent(action, x, y, eventTime, this, this.mKeyboard);
            } else {

...
import static com.nvidia.inputmethod.latin.Constants.ImeOption.FORCE_ASCII;
import static com.nvidia.inputmethod.latin.Constants.ImeOption.NO_MICROPHONE;
import static com.nvidia.inputmethod.latin.Constants.ImeOption.NO_MICROPHONE_COMPAT;
...
import com.nvidia.inputmethod.accessibility.AccessibilityUtils;
import com.nvidia.inputmethod.accessibility.AccessibleKeyboardViewProxy;
import com.nvidia.inputmethod.compat.CompatUtils;
import com.nvidia.inputmethod.compat.InputMethodManagerCompatWrapper;
import com.nvidia.inputmethod.compat.InputMethodServiceCompatUtils;
import com.nvidia.inputmethod.compat.SuggestionSpanUtils;
...
import com.nvidia.inputmethod.keyboard.MainKeyboardView;
import com.nvidia.inputmethod.latin.LocaleUtils.RunInLocale;
import com.nvidia.inputmethod.latin.Utils.Stats;
import com.nvidia.inputmethod.latin.define.ProductionFlag;
import com.nvidia.inputmethod.latin.suggestions.SuggestionStripView;
...
public final class LatinIME extends InputMethodService implements KeyboardActionListener,
    ...
    public static final int smallOffset = 2; //this is a small offset that is added in the x and y direction from the corner of all keys to ensure the touch events occur within the key boundry

    //all touch events are tracked by indexes, because the device can handle at most 10 touch events, we will track the programmatic events with the values 11-16
    static final int aTouchIndex = 11;
    static final int xTouchIndex = 12;
    static final int yTouchIndex = 13;
    static final int startTouchIndex = 14;
    static final int shiftTouchIndex = 15;
    static final int alphaTouchIndex = 16;
    public static final int moreKeysTouchIndex = 17;

    static final int KeyboardFocus = 0;
    static final int SuggestionsFocus = 1;
    static final int AddToDictionaryFocus = 2;

    private int mJoyFocus = KeyboardFocus;
    ...
    @Override
    public void onStartInput(final EditorInfo editorInfo, final boolean restarting) {
        if (mJoyFocus != KeyboardFocus) {
            regainKeyboardFocus(false);
        }
    ...
    public void onStartInputView(final EditorInfo editorInfo, final boolean restarting) {
        ...
        if (mKeyboardSwitcher != null) {
            Keyboard t = mKeyboardSwitcher.getKeyboard();
            if (t != null) {
                t.selectShortcut();
            }
        }
    ...
    private void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        if (TRACE) Debug.startMethodTracing("/data/trace/nvlatinime");
    }
    ...
    public void pickSuggestionManually(final int index, final CharSequence suggestion) {
        ...
        if (showingAddToDictionaryHint && mIsUserDictionaryAvailable) {
            mJoyFocus = AddToDictionaryFocus;
    ...
    private void handleShiftDown() {
        if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SHIFT) && !Keyboard.lThumbPressed) {
            MainKeyboardView keyboardView = mKeyboardSwitcher.getKeyboardView();
            PointerTracker tracker = PointerTracker.getPointerTracker(shiftTouchIndex, keyboardView);
            Key press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_SHIFT);
            tracker.onDownEvent(press.mX+2, press.mY+2, System.currentTimeMillis(), keyboardView, null);
        }
    }
    private void handleShiftUp() {
        if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SHIFT) && !Keyboard.lThumbPressed) {
            ...
            PointerTracker tracker = PointerTracker.getPointerTracker(shiftTouchIndex, keyboardView);
            Key press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_SHIFT);
            tracker.onUpEvent(press.mX+2, press.mY+2, System.currentTimeMillis());
        }
    }

    private void loseSuggestionsFocus(){
        mSuggestionStripView.deselect();
        mJoyFocus = KeyboardFocus;
        dismissAddWord();
    }
    private void longClickSuggestion() {
        mSuggestionStripView.longPressFired = mSuggestionStripView.getSelectedView().performLongClick();
    }
    private void regainKeyboardFocus(boolean fromSuggestion) {
        loseSuggestionsFocus();
        if(fromSuggestion){
            int s = mSuggestionStripView.getSelected();
            mKeyboardSwitcher.getKeyboard().gainFocus(s);
        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (this.isInputViewShown()) {
            Key press = null;
            MainKeyboardView keyboardView = mKeyboardSwitcher.getKeyboardView();
            PointerTracker tracker = null;
            switch (keyCode) {
                //handle a button
                case KeyEvent.KEYCODE_BUTTON_A:
                    if (!Keyboard.aPressed) {
                        if (mJoyFocus == KeyboardFocus) {
                            if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                                press = mKeyboardSwitcher.getKeyboard().getSelected();
                                tracker = PointerTracker.getPointerTracker(aTouchIndex, keyboardView);
                                tracker.onDownEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis(), keyboardView, keyboardView.getKeyboard());
                            }
                        } else if (mJoyFocus == SuggestionsFocus) {
                            mSuggestionStripView.aPressedTime = SystemClock.uptimeMillis();
                        } else if (mJoyFocus == SuggestionsFocus) {

                        }
                        Keyboard.aPressed = true;
                    } else if (mJoyFocus == SuggestionsFocus && !mSuggestionStripView.longPressFired && (SystemClock.uptimeMillis()-mSuggestionStripView.aPressedTime) > ViewConfiguration.getLongPressTimeout()){
                ...
                case KeyEvent.KEYCODE_BUTTON_X:
                    ...
                    mJoyFocus = KeyboardFocus;
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_DELETE) && !Keyboard.delPressed) {
                        Keyboard.delPressed = true;
                        tracker = PointerTracker.getPointerTracker(xTouchIndex, keyboardView);
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_DELETE);
                        tracker.onDownEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis(), keyboardView, null);
                    }
                    return true;
                ...
                case KeyEvent.KEYCODE_BUTTON_Y:
                    ...
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SPACE) && !Keyboard.spacePressed) {
                        Keyboard.spacePressed = true;
                        tracker = PointerTracker.getPointerTracker(yTouchIndex, keyboardView);
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_SPACE);
                        tracker.onDownEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis(), keyboardView, null);
                    }
                    return true;
                ...
                case KeyEvent.KEYCODE_BUTTON_START:
                    ...
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_ACTION_ENTER)) {
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_ACTION_ENTER);
                    } else if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_ACTION_NEXT)) {
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_ACTION_NEXT);
                    } else if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_ENTER)) {
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_ENTER);
                    } else {
                        return true;
                    }
                    if (press != null){
                        tracker = PointerTracker.getPointerTracker(startTouchIndex, keyboardView);
                        tracker.onDownEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis(), keyboardView, null);
                    }
                    return true;
                ...
                case KeyEvent.KEYCODE_DPAD_UP:
                    if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                        ...
                        if (mJoyFocus == KeyboardFocus) {
                            if (mKeyboardSwitcher.getKeyboard().atTop() && mSuggestionStripView.getSuggestionCount() > 0) {
                                //move focus to suggested words
                                int sugIndex = mKeyboardSwitcher.getKeyboard().getSuggestedIndex();
                                if(isSuggestionsStripVisible() && mSuggestionStripView.selectIndex(sugIndex)) {
                                    mJoyFocus = SuggestionsFocus;
                                    mKeyboardSwitcher.getKeyboard().leaveFocus();
                                    mKeyboardSwitcher.getKeyboardView().invalidateAllKeys();
                                }
                            } else {
                                mKeyboardSwitcher.getKeyboard().selectUp();
                                mKeyboardSwitcher.getKeyboardView().invalidateAllKeys();
                            }
                        }
                        mKeyboardSwitcher.getKeyboard().mCanLoopVertical = false;
                    } else {
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().selectUp();
                        mKeyboardSwitcher.getKeyboardView().invalidateMoreKeysView();
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopVertical = false;
                    }
                    return true;
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                        ...
                        if (mJoyFocus == KeyboardFocus) {
                            mKeyboardSwitcher.getKeyboard().selectDown();
                            mKeyboardSwitcher.getKeyboardView().invalidateAllKeys();
                        } else if (mJoyFocus == SuggestionsFocus || mJoyFocus == AddToDictionaryFocus) {
                            regainKeyboardFocus(true);
                        }
                        mKeyboardSwitcher.getKeyboard().mCanLoopVertical = false;
                    } else {
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().selectDown();
                        mKeyboardSwitcher.getKeyboardView().invalidateMoreKeysView();
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopVertical = false;
                    }
                    return true;
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                        if (mJoyFocus == KeyboardFocus) {
                            mKeyboardSwitcher.getKeyboard().selectLeft();
                            mKeyboardSwitcher.getKeyboardView().invalidateAllKeys();
                            mKeyboardSwitcher.getKeyboard().mCanLoopHorizontal = false;
                        }  else if (mJoyFocus == SuggestionsFocus) {
                            mSuggestionStripView.selectLeft();
                        }
                    } else {
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().selectLeft();
                        mKeyboardSwitcher.getKeyboardView().invalidateMoreKeysView();
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopHorizontal = false;
                    }
                    return true;
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                        if (mJoyFocus == KeyboardFocus) {
                            mKeyboardSwitcher.getKeyboard().selectRight();
                            mKeyboardSwitcher.getKeyboardView().invalidateAllKeys();
                            mKeyboardSwitcher.getKeyboard().mCanLoopHorizontal = false;
                        } else if (mJoyFocus == SuggestionsFocus) {
                            mSuggestionStripView.selectRight();
                        }
                    } else {
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().selectRight();
                        mKeyboardSwitcher.getKeyboardView().invalidateMoreKeysView();
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopHorizontal = false;
                    }
                    return true;
                ...
                case KeyEvent.KEYCODE_BUTTON_SELECT:
                    ...
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SWITCH_ALPHA_SYMBOL) && !Keyboard.alphaPressed) {
                        Keyboard.alphaPressed = true;
                        tracker = PointerTracker.getPointerTracker(alphaTouchIndex, keyboardView);
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_SWITCH_ALPHA_SYMBOL);
                        tracker.onDownEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis(), keyboardView, null);
                    }
                    return true;
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (this.isInputViewShown()) {
            ...
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_BUTTON_A:
                    ...
                    if (mJoyFocus == KeyboardFocus) {
                        if (!mKeyboardSwitcher.getKeyboardView().moreKeysShowing()) {
                            tracker = PointerTracker.getPointerTracker(aTouchIndex, keyboardView);
                            press = mKeyboardSwitcher.getKeyboard().getSelected();
                            tracker.onUpEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis());
                        } else {
                            tracker = PointerTracker.getPointerTracker(aTouchIndex, keyboardView);
                            press = mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().getSelected();
                            tracker.onMoveEvent(press.mX + smallOffset, press.mY + press.mHeight, System.currentTimeMillis(), null);
                            tracker.onUpEvent(press.mX + smallOffset, press.mY + press.mHeight, System.currentTimeMillis());
                        }
                        mSuggestionStripView.longPressFired = false;
                    } else if (mJoyFocus == SuggestionsFocus){
                        if ((SystemClock.uptimeMillis() - mSuggestionStripView.aPressedTime) > ViewConfiguration.getLongPressTimeout()) {
                            longClickSuggestion();
                        } else {
                            mSuggestionStripView.getSelectedView().performClick();
                        }
                    } else if (mJoyFocus == AddToDictionaryFocus){
                        mSuggestionStripView.addwordToDictionary();
                    }
                    mSuggestionStripView.longPressFired = false;
                    return true;
                case KeyEvent.KEYCODE_BUTTON_X:
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_DELETE)) {
                        Keyboard.delPressed = false;
                        tracker = PointerTracker.getPointerTracker(xTouchIndex, keyboardView);
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_DELETE);
                        tracker.onUpEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis());
                    }
                ...
                case KeyEvent.KEYCODE_BUTTON_Y:
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SPACE)) {
                ...
                case KeyEvent.KEYCODE_BUTTON_THUMBL:
                    Keyboard.lThumbPressed = false;
                    handleShiftUp();
                    return true;

                case KeyEvent.KEYCODE_BUTTON_THUMBR:
                case KeyEvent.KEYCODE_BUTTON_SELECT:
                    if (mKeyboardSwitcher.getKeyboard().hasKey(Keyboard.CODE_SWITCH_ALPHA_SYMBOL)) {
                        Keyboard.alphaPressed = false;
                        tracker = PointerTracker.getPointerTracker(alphaTouchIndex, keyboardView);
                        press = mKeyboardSwitcher.getKeyboard().getKey(Keyboard.CODE_SWITCH_ALPHA_SYMBOL);
                        tracker.onUpEvent(press.mX + smallOffset, press.mY + smallOffset, System.currentTimeMillis());
                    }
                ...
                case KeyEvent.KEYCODE_DPAD_UP:
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    if (mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard() != null)
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopVertical = true;
                    mKeyboardSwitcher.getKeyboard().mCanLoopVertical = true;
                    return true;
                case KeyEvent.KEYCODE_DPAD_LEFT:
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    if (mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard() != null)
                        mKeyboardSwitcher.getKeyboardView().getMoreKeysKeyboard().mCanLoopHorizontal = true;
                    mKeyboardSwitcher.getKeyboard().mCanLoopHorizontal = true;
                    return true;
                default:
                    return super.onKeyUp(keyCode, event);
            }

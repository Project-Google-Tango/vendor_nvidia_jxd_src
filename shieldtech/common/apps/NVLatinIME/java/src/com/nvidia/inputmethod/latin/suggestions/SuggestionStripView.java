...
import com.nvidia.inputmethod.keyboard.ViewLayoutUtils;
import com.nvidia.inputmethod.latin.AutoCorrection;
...
import com.nvidia.inputmethod.latin.StaticInnerHandlerWrapper;
...
public final class SuggestionStripView extends RelativeLayout implements OnClickListener,
    ...
    // The maximum number of suggestions available. See {@link Suggest#mPrefmSuggestionCount}.
    public static final int MAX_SUGGESTIONS = 18;
    static final String TAG = "SUGGESTIONS_STRIP_VIEW";
    ...
    public long aPressedTime = 0;
    public boolean longPressFired = false;
    ...
    public int getSuggestionCount() {
        return mParams.getSuggestionCount();
    }

    public boolean selectIndex (int i) {
        return mParams.selectIndex(i);
    }
    ...
    public TextView getSelectedView() {
        return mParams.getSelectedView();
    }

    public void selectRight() {
        mParams.moveSelection(1);
    }
    ...
    private static final class SuggestionStripViewParams {
        ...
        public boolean mWillAutoCorrect = false;
        ...
        public TextView getSelectedView() {
            return mWords.get(indexTransform(selectedIndex));
        }
        private int indexTransform(int i)
        {
            if (i < 2) {
                if (!mWillAutoCorrect) {
                    if (i == 0) {
        ...
        public boolean canSelect(int i) {
            i = indexTransform(i);
            return mWords.get(i).getText().length() > 0;
        }

        public boolean selectIndex (int i) {
            //store the selected index in screen position before it is transformed
            while (!canSelect(i)){
                i--;
                if (i < 0){
                    return false;
        ...
        public void moveSelection(int i) {
            deselect();
            selectedIndex += i;
            int sCount = getSuggestionCount();
            if (selectedIndex >= sCount) {
                selectedIndex = sCount-1;
            } else if(selectedIndex < 0) {
                selectedIndex = 0;
            }
            int t = indexTransform(selectedIndex);
            TextView temp = mWords.get(t);
            if (temp != null) {
                temp.setSelected(true);
            }
        }
        ...
        public int getSuggestionCount() {
            int count = 0;
            for (int i = 0; i < mSuggestionCount; i++) {
                if (mWords.get(i).getText().length() > 0){
                    count++;
                }
            }
            return count;

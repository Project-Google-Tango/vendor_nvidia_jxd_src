...
import static com.nvidia.inputmethod.keyboard.Keyboard.CODE_OUTPUT_TEXT;
import static com.nvidia.inputmethod.keyboard.Keyboard.CODE_SHIFT;
import static com.nvidia.inputmethod.keyboard.Keyboard.CODE_SWITCH_ALPHA_SYMBOL;
import static com.nvidia.inputmethod.keyboard.Keyboard.CODE_UNSPECIFIED;
import static com.nvidia.inputmethod.keyboard.internal.KeyboardIconsSet.ICON_UNDEFINED;
...
import com.nvidia.inputmethod.keyboard.internal.KeyDrawParams;
import com.nvidia.inputmethod.keyboard.internal.KeySpecParser;
import com.nvidia.inputmethod.keyboard.internal.KeyStyle;
...
import com.nvidia.inputmethod.keyboard.internal.KeyboardRow;
...
public class Key implements Comparable<Key> {
    ...
    /** The current selected state of the key */
    ...
    /**
     * Informs the key that it has been selected, in case it needs to change its appearance or
     * state.
     */
    ...
    public final int[] getCurrentDrawableState() {
        ...
        switch (mBackgroundType) {
            case BACKGROUND_TYPE_FUNCTIONAL:
                ret = mPressed ? KEY_STATE_FUNCTIONAL_PRESSED : KEY_STATE_FUNCTIONAL_NORMAL;
                break;
            case BACKGROUND_TYPE_ACTION:
                ret = mPressed ? KEY_STATE_ACTIVE_PRESSED : KEY_STATE_ACTIVE_NORMAL;
                break;
            case BACKGROUND_TYPE_STICKY_OFF:
                ret = mPressed ? KEY_STATE_PRESSED_HIGHLIGHT_OFF : KEY_STATE_NORMAL_HIGHLIGHT_OFF;
                break;
            case BACKGROUND_TYPE_STICKY_ON:
                ret = mPressed ? KEY_STATE_PRESSED_HIGHLIGHT_ON : KEY_STATE_NORMAL_HIGHLIGHT_ON;
                break;
            default: /* BACKGROUND_TYPE_NORMAL */
                ret = mPressed ? KEY_STATE_PRESSED : KEY_STATE_NORMAL;
                break;
        }
        if (mSelected) {
            int [] r2 = new int[ret.length+1];
            for (int i = 0; i < ret.length; i++) {
                r2[i] = ret[i];
            }
            r2[r2.length-1] = android.R.attr.state_focused;
            return r2;
        }
        return ret;
    }

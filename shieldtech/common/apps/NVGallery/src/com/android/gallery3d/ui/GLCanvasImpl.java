...
public class GLCanvasImpl implements GLCanvas {
    ...
    private boolean mHasDeferredClear = false;
    private float[] mDeferredClearColor = new float[4];
    ...
    public void drawRect(float x, float y, float width, float height, GLPaint paint) {
        clearIfNeeded(null, x, y, width, height);
    ...
    public void drawTexture(
        ...
        clearIfNeeded(texture, x, y, width, height);
    ...
    public void clearBuffer(float[] argb) {
        if(argb != null && argb.length == 4) {
            mDeferredClearColor[0] = argb[1];
            mDeferredClearColor[1] = argb[2];
            mDeferredClearColor[2] = argb[3];
            mDeferredClearColor[3] = argb[0];
        } else {
            mDeferredClearColor[0] = 0.0f;
            mDeferredClearColor[1] = 0.0f;
            mDeferredClearColor[2] = 0.0f;
            mDeferredClearColor[3] = 1.0f;
        }
        mHasDeferredClear = true;
    }
    ...
    private void flushDeferredClear() {
        mGL.glClearColor(mDeferredClearColor[0],
                         mDeferredClearColor[1],
                         mDeferredClearColor[2],
                         mDeferredClearColor[3]);
        mGL.glClear(GL10.GL_COLOR_BUFFER_BIT);
        mHasDeferredClear = false;
    }

    private void clearIfNeeded(BasicTexture texture,
                     float x, float y, float width, float height) {
        boolean scissored;

        // do nothing if there's no pending clear
        if (!mHasDeferredClear)
            return;

        scissored = mGL.glIsEnabled(GL11.GL_SCISSOR_TEST);

        // SCISSOR_TEST is not in use in this code base but SCISSOR_TEST
        // check is added here in case someone use scissor in the future
        // If scissor is enabled, rect dimention should be checked to
        // safely skip redundant glclear
        if (scissored) {
            Log.w(TAG, "GL_SCISSOR_TEST is enabled. deferred glclear might not work");
        }

        // check if the texture covers entire screen
        if ((x == 0) && (y == 0) && (width == mScreenWidth) && (height == mScreenHeight)) {
            boolean opaque;

            if (texture == null) {
                opaque = true;
            } else {
                opaque = texture.isOpaque() && mAlpha >= OPAQUE_ALPHA;
            }

            // check if we there's blending
            if ((mBlendEnabled && !opaque) || scissored) {
                flushDeferredClear();
            }
        } else {
            flushDeferredClear();

...
public class PhotoPage extends ActivityState implements
    ...
    public void onCreate(Bundle data, Bundle restoreState) {
        ...
        mActionBar.disableActionBarFocus();
        mCurrentPos = Position.MIDDLE;
        ...
        if (mSetPathString != null) {
            ...
            pda.setDataListener(new PhotoDataAdapter.DataListener() {
                ...
                public void onPhotoChanged(int index, Path item) {
                    ...
                    if (mCurrentPos != Position.MIDDLE)
                        refreshHidingMessage();
                }
    ...
    private Uri checkAndGetMediaContentProviderUri(Uri imageContentProviderUri)
    {
        if(imageContentProviderUri.getAuthority().equals("downloads"))
        {
            Cursor cursor = null;
            try
            {
                cursor = mActivity.getAndroidContext().getContentResolver().query(imageContentProviderUri,
                        new String[] {
                        "mediaprovider_uri"
                },
                null, null, null);
                ...
                if(cursor == null)
                        return imageContentProviderUri;
    ...
    private void launchPhotoEditor() {
        ...
        Uri final_image_uri = checkAndGetMediaContentProviderUri(current.getContentUri());

        Intent intent = new Intent(ACTION_NEXTGEN_EDIT);

        intent.setDataAndType(final_image_uri, current.getMimeType())
                .setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
    ...
    private void showBars() {
        mActionBar.enableActionBarFocus();
        mCurrentPos = Position.TOP;
        refreshHidingMessage();
        if (mShowBars) return;
        mShowBars = true;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        float flingspeed=301;
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_UP: // up
                if (!mPhotoView.getFilmMode()) {
                    if (mActionBar.isActionBarFocused() || mShowBars) {
                        refreshHidingMessage();
                        if (mCurrentPos == Position.MIDDLE) {
                            mActionBar.enableActionBarFocus();
                            mCurrentPos = Position.TOP;
                        }
                        else if (mCurrentPos == Position.BOTTOM) {
                            mActionBar.disableActionBarFocus();
                            mCurrentPos = Position.MIDDLE;
                        }
                    }
                ...
                else if (!mActionBar.isActionBarFocused())
                    mActionBar.enableActionBarFocus();
                break;
            case KeyEvent.KEYCODE_DPAD_DOWN: //down
                if (mPhotoView.getFilmMode()) {
                    ...
                    if (mActionBar.isActionBarFocused() || mShowBars) {
                        refreshHidingMessage();
                        if (mCurrentPos == Position.MIDDLE &&
                                mBottomControls.isVisible()) {
                            mActionBar.enableActionBarFocus();
                            mCurrentPos = Position.BOTTOM;
                        }
                        else if (mCurrentPos == Position.TOP) {
                            mActionBar.disableActionBarFocus();
                            mCurrentPos = Position.MIDDLE;
                        }
                    }
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                //left
                if (mActionBar.isActionBarFocused() && (mCurrentPos != Position.MIDDLE)) {
                    if (!mPhotoView.getFilmMode()) refreshHidingMessage();
                    return false;
                }
                else mPhotoView.moveJoy(-1);
                break;
            ...
            case KeyEvent.KEYCODE_DPAD_CENTER:
                // d-pad center
                if (mActionBar.isActionBarFocused()) {
                    if (!mPhotoView.getFilmMode()) {
                        refreshHidingMessage();
                    }
                }
            ...
            case KeyEvent.KEYCODE_BUTTON_A:
                ...
                if (playVideo) {
                    ...
                    // Photo: put into full screen
                    mPhotoView.setFilmMode(false);
                }
                break;
            case KeyEvent.KEYCODE_BUTTON_L1:
                //left button
                if (mActionBar.isActionBarFocused()) wantBars();
                else mPhotoView.moveJoy(-1);
                break;

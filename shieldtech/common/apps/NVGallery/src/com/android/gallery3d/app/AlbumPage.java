...
public class AlbumPage extends ActivityState implements GalleryActionBar.ClusterRunner,
    ...
    private int currentPictureFocused = 0;
    ...
    private void onSingleTapUp(int slotIndex) {
        ...
        if (mSelectionManager.inSelectionMode()) {
            ...
            currentPictureFocused = slotIndex;
            mAlbumView.setSelected(currentPictureFocused);
    ...
    protected void onResume() {
        ...
        mSlotView.setCenterIndex(currentPictureFocused);
    }
    ...
    public boolean onKeyLongPress(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BUTTON_A) {
            onLongTap(currentPictureFocused);
            return true;
        }
        return false;
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_UP:
                //up
                if (currentPictureFocused % 2 == 1) {
                    currentPictureFocused -= 1;
                }
                else {
                    mActivity.getGalleryActionBar().enableActionBarFocus();
                }
                break;
            case KeyEvent.KEYCODE_DPAD_DOWN:
                //down
                if (mActivity.getGalleryActionBar().isActionBarFocused())
                    mActivity.getGalleryActionBar().disableActionBarFocus();
                else if (currentPictureFocused % 2 == 0) {
                    currentPictureFocused += 1;
                }
                break;
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                //left
                if (currentPictureFocused >1 ) {
                    currentPictureFocused -= 2;
                }
                break;
            case KeyEvent.KEYCODE_BUTTON_R1:
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                //right
                if (mAlbumDataAdapter.size() > currentPictureFocused + 2) {
                    currentPictureFocused += 2;
                }
                break;
        ...
        if (mAlbumDataAdapter.size() <= currentPictureFocused) {
            currentPictureFocused = mAlbumDataAdapter.size()-1;
        }

        if (mActivity.getGalleryActionBar().isActionBarFocused())
            mAlbumView.setSelected(-1);
        else
            mAlbumView.setSelected(currentPictureFocused);
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (!event.isCanceled()) {
            switch (keyCode) {
                case KeyEvent.KEYCODE_BUTTON_A:
                    // A button
                    if (mButtonPressed) { // We may capture a key up from when the previous state
                        onSingleTapUp(currentPictureFocused);
